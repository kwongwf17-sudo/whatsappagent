...
test("purgeAccountData removes only the selected account from json storage", async () => {
  const dataDir = await mkdtemp(path.join(tmpdir(), "wa-store-"));
  const store = new JsonStore(dataDir);

  await store.getOrCreateCustomer("6731111111", { businessAccountId: "store-a", productId: "sku-a" });
  await store.getOrCreateCustomer("6732222222", { businessAccountId: "store-b", productId: "sku-b" });
  await store.appendOutbox({ direction: "inbound", from: "6731111111", businessAccountId: "store-a", body: "hi" });
  await store.appendOutbox({ direction: "inbound", from: "6732222222", businessAccountId: "store-b", body: "hi" });
  await store.addOrder({ id: "order-a", customerId: "6731111111", businessAccountId: "store-a", productId: "sku-a" });
  await store.addOrder({ id: "order-b", customerId: "6732222222", businessAccountId: "store-b", productId: "sku-b" });
  await store.claimProcessedMessage("msg-a", "store-a");
  await store.claimProcessedMessage("msg-b", "store-b");
  await store.savePendingBuffer("store-a:6731111111", { businessAccountId: "store-a", customerId: "6731111111" });
  await store.savePendingBuffer("store-b:6732222222", { businessAccountId: "store-b", customerId: "6732222222" });
  await store.appendAuditLog({ actor: "admin:store-a", businessAccountId: "store-a", action: "test" });
  await store.appendAuditLog({ actor: "admin:store-b", businessAccountId: "store-b", action: "test" });
  await writeFile(path.join(dataDir, "deleted_customers.json"), JSON.stringify({
    customers: [
      { id: "6731111111", businessAccountId: "store-a" },
      { id: "6732222222", businessAccountId: "store-b" },
    ],
  }));

  const summary = await store.purgeAccountData("store-a");
  assert.equal(summary.customers, 1);
  assert.equal(summary.orders, 1);
  assert.equal(summary.outbox, 1);
  assert.equal(summary.processedMessages, 1);
  assert.equal(summary.pendingBuffers, 1);
  assert.equal(summary.auditEvents, 1);
  assert.equal(summary.deletedCustomers, 1);

  assert.deepEqual((await store.listCustomers(new Date(), "store-a")), []);
  assert.equal((await store.listCustomers(new Date(), "store-b")).length, 1);
  assert.equal((await store.listOrders("store-a")).length, 0);
  assert.equal((await store.listOrders("store-b")).length, 1);
  assert.equal((await store.listOutbox("store-a")).length, 0);
  assert.equal((await store.listOutbox("store-b")).length, 1);
  assert.equal(await store.getProcessedMessage("msg-a", "store-a"), null);
  assert.ok(await store.getProcessedMessage("msg-b", "store-b"));
  assert.equal(await store.getPendingBuffer("store-a:6731111111"), null);
  assert.ok(await store.getPendingBuffer("store-b:6732222222"));
  const deleted = JSON.parse(await readFile(path.join(dataDir, "deleted_customers.json"), "utf8"));
  assert.deepEqual(deleted.customers.map((customer) => customer.businessAccountId), ["store-b"]);
});
...
test("table-backed orders avoid rewriting orders JSON and stay account scoped", async () => {
  const dataDir = await mkdtemp(path.join(tmpdir(), "wa-store-"));
  const adapter = new MemoryTableAdapter();
  const store = new JsonStore(dataDir, { adapter });

  await store.getOrCreateCustomer("6731111111", { businessAccountId: "ALLEN", productId: "product-a" });
  await store.getOrCreateCustomer("6731111111", { businessAccountId: "allen", productId: "product-b" });
  const upperOrder = await store.addOrder({ businessAccountId: "ALLEN", customerId: "6731111111", productId: "product-a" });
  await store.addOrder({ businessAccountId: "allen", customerId: "6731111111", productId: "product-b" });

  assert.deepEqual((await store.listOrders("ALLEN")).map((order) => order.productId), ["product-a"]);
  await store.updateOrderStatus(upperOrder.id, "reached_warehouse", "admin");
  assert.equal((await store.listOrders("ALLEN"))[0].status, "reached_warehouse");
  await store.deleteCustomer("6731111111", "test", new Date("2026-01-01T00:00:00.000Z"), "ALLEN");

  assert.equal((await store.listOrders("ALLEN")).length, 0);
  assert.equal((await store.listOrders("allen")).length, 1);
  assert.equal(adapter.writeCounts.get("orders") || 0, 0);
});

test("table-backed processed messages and pending buffers avoid hot JSON writes", async () => {
  const dataDir = await mkdtemp(path.join(tmpdir(), "wa-store-"));
  const adapter = new MemoryTableAdapter();
  const store = new JsonStore(dataDir, { adapter });

  const claimed = await store.claimProcessedMessage("wamid_1", "ALLEN", { source: "web" });
  const duplicate = await store.claimProcessedMessage("wamid_1", "ALLEN");
  await store.completeProcessedMessage("wamid_1", "ALLEN");
  await store.savePendingBuffer("ALLEN::673::merge", { businessAccountId: "ALLEN", customerId: "673", type: "merge", dueAt: "2026-01-01T00:00:00.000Z" });
  await store.updatePendingBuffer("ALLEN::673::merge", (buffer) => ({ ...buffer, fragments: ["hi"] }));

  assert.equal(claimed.claimed, true);
  assert.equal(duplicate.claimed, false);
  assert.equal((await store.getProcessedMessage("wamid_1", "ALLEN")).processingStatus, "completed");
  assert.deepEqual((await store.listPendingBuffers("ALLEN"))[0].fragments, ["hi"]);
  assert.equal(adapter.writeCounts.get("processed_messages") || 0, 0);
  assert.equal(adapter.writeCounts.get("pending_buffers") || 0, 0);
  assert.equal(await store.deletePendingBuffer("ALLEN::673::merge"), true);
});

test("table-backed audit log avoids audit JSON rewrites", async () => {
  const dataDir = await mkdtemp(path.join(tmpdir(), "wa-store-"));
  const adapter = new MemoryTableAdapter();
  const store = new JsonStore(dataDir, { adapter });

  await store.appendAuditLog({ businessAccountId: "ALLEN", action: "opening_flow_sent" });
  await store.appendAuditLog({ businessAccountId: "allen", action: "opening_flow_sent" });

  assert.deepEqual((await store.listAuditLog()).map((event) => event.businessAccountId), ["ALLEN", "allen"]);
  assert.equal(adapter.writeCounts.get("audit_log") || 0, 0);
});
...
class MemoryTableAdapter {
  constructor() {
    ...
    this.ordersTableEnabled = true;
    this.processedMessagesTableEnabled = true;
    this.pendingBuffersTableEnabled = true;
    this.auditEventsTableEnabled = true;
    this.outbox = [];
  ...
  async listOrderRows(businessAccountId = "") {
    return [...this.orders.values()]
      .filter((order) => !businessAccountId || order.businessAccountId === businessAccountId)
  ...
  async deleteOrderRowsForCustomer(customerId, businessAccountId = "") {
    ...
    for (const [id, order] of this.orders.entries()) {
      if (order.customerId === customerId && (!businessAccountId || order.businessAccountId === businessAccountId)) {
  ...
  async claimProcessedMessageRow(messageId, businessAccountId = "", metadata = {}) {
    const key = `${businessAccountId}::${messageId}`;
  ...
  async updateProcessedMessageRow(messageId, businessAccountId = "", patch = {}) {
    const key = `${businessAccountId}::${messageId}`;
    const existing = this.processedMessages.get(key) || { businessAccountId, messageId, receivedAt: new Date().toISOString() };
  ...
  async getProcessedMessageRow(messageId, businessAccountId = "") {
    return structuredClone(this.processedMessages.get(`${businessAccountId}::${messageId}`) || null);
  }

  async upsertPendingBufferRow(buffer = {}) {
    this.pendingBuffers.set(buffer.key, structuredClone(buffer));
    return structuredClone(buffer);
  }

  async getPendingBufferRow(key) {
    return structuredClone(this.pendingBuffers.get(key) || null);
  }

  async listPendingBufferRows(businessAccountId = "") {
    return [...this.pendingBuffers.values()]
      .filter((buffer) => !businessAccountId || buffer.businessAccountId === businessAccountId)
      .map((buffer) => structuredClone(buffer));
  }

  async deletePendingBufferRow(key) {
    return this.pendingBuffers.delete(key);
  }

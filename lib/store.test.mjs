...
test("renamed follow-up rows respect legacy sent keys", async () => {
  const dataDir = await mkdtemp(path.join(tmpdir(), "wa-store-"));
  const store = new JsonStore(dataDir);
  await store.getOrCreateCustomer("6731111111", {
    businessAccountId: "store-a",
    ...
    orderIds: [],
    followupsSent: {
      day_1_followup: "2026-07-01T04:00:00.000Z",
    },
  });
    ...
    products: [
      {
        id: "wipe-xpert",
        followup_schedule: [
          { key: "first_day_followup_2", legacy_keys: ["day_1_followup"], timing_type: "delay_after_opening", delay_duration: "04:00", message: "Old row renamed" },
          { key: "first_day_followup_3", timing_type: "delay_after_opening", delay_duration: "07:00", message: "Next row" },
        ],
      },
    ],
  };

  const due = await store.getDueFollowups(catalog, new Date("2026-07-01T07:00:00.000Z"));
  assert.equal(due[0].followupKey, "first_day_followup_3");
});

test("detailed follow-up sent history gates later stages", async () => {
  const dataDir = await mkdtemp(path.join(tmpdir(), "wa-store-"));
  const store = new JsonStore(dataDir);
  await store.getOrCreateCustomer("6731111111", {
    businessAccountId: "store-a",
  ...
  await store.markFollowupSent("6731111111", "first_day_followup_1", new Date("2026-07-01T04:00:00.000Z"), "store-a", {
    label: "First Follow-Up",
    ...
    products: [
      {
        id: "wipe-xpert",
        followup_schedule: [
          { key: "first_day_followup_1", timing_type: "delay_after_opening", delay_duration: "04:00", message: "First row" },
          { key: "first_day_followup_2", timing_type: "delay_after_opening", delay_duration: "07:00", message: "Second row" },
        ],
      },
    ],
  };
  ...
  const due = await store.getDueFollowups(catalog, new Date("2026-07-01T07:00:00.000Z"));
  assert.equal(due[0].followupKey, "first_day_followup_2");
  const customer = await store.getCustomer("6731111111", "store-a");
  assert.equal(customer.followupsSent.first_day_followup_1.label, "First Follow-Up");
  assert.equal(customer.followupsSent.first_day_followup_1.sentAt, "2026-07-01T04:00:00.000Z");
});

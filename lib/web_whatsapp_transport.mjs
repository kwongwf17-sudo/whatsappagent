...
function messageMediaMimeType(message = {}) {
  const content = unwrapMessageContent(message.message || {});
  return (
...
function messageCaption(message = {}) {
  const content = unwrapMessageContent(message.message || {});
  return content.imageMessage?.caption || content.videoMessage?.caption || "";
}

function hasDownloadableMedia(message = {}) {
  const content = unwrapMessageContent(message.message || {});
  const media = content.audioMessage || content.imageMessage || content.videoMessage || {};
  return Boolean(media.mediaKey && (media.directPath || media.url));
}
...
export class WebWhatsAppTransport {
  constructor({
    ...
    this.qrTimeoutMs = Math.max(0, Number(qrTimeoutMs) || 0);
    this.sock = null;
  ...
  getStatus() {
    return {
      ...
      hasQrTimeoutTimer: Boolean(this.qrTimeoutTimer),
      qrStartedAt: this.qrStartedAt,
  ...
  async start({ onMessage, onRecoveredMessage } = {}) {
    if (this.started) return;
    this.started = true;
    this.onMessage = onMessage;
    this.onRecoveredMessage = onRecoveredMessage || null;
    this.manualDisconnect = false;
    this.clearQrTimeout();
    ...
    try {
      ...
      this.sock.ev.on("connection.update", (update) => {
        ...
        if (qr) {
          ...
          this.scheduleQrTimeout();
          this.logger.log("WhatsApp Web QR ready. Open /admin/whatsapp-web to scan.");
        }
        if (connection === "open") {
          const previousDisconnectedAt = this.lastDisconnectedAt;
          this.clearReconnectTimer();
          this.clearQrTimeout();
          ...
          this.recoverySinceAt = previousDisconnectedAt || "";
          this.lastError = "";
      ...
      this.sock.ev.on("messaging-history.set", (event) => {
        void this.handleRecoveredHistory(event).catch((error) => {
          this.lastError = error.message;
          this.logger.error("WhatsApp Web recovery sync failed:", error);
        });
      });
    } catch (error) {
      this.status = "error";
  ...
  scheduleQrTimeout() {
    if (!this.qrTimeoutMs || this.qrTimeoutTimer) return;
    const now = Date.now();
    this.qrStartedAt ||= new Date(now).toISOString();
    this.qrExpiresAt = new Date(now + this.qrTimeoutMs).toISOString();
    this.qrTimeoutTimer = setTimeout(() => {
      this.qrTimeoutTimer = null;
      void this.expireQrSession();
    }, this.qrTimeoutMs);
  }
  ...
  async expireQrSession() {
    if (this.status !== "qr_required" && this.status !== "pairing_code_ready") return;
    ...
    this.status = "qr_expired";
    this.lastDisconnectedAt = new Date().toISOString();
    this.lastError = "QR session expired before scan";
    this.logger.warn("WhatsApp Web QR expired before scan; transport stopped.");
  }
  ...
  closeSocket() {
    ...
    try {
      ...
      // Baileys event emitters vary by version; best-effort cleanup.
    }
    try {
      sock.ws?.close?.();
    } catch {
      // Best-effort socket close before reconnect.
    }
    try {
      sock.end?.();
    } catch {
  ...
  async handleMessages(event = {}) {
    ...
    for (const item of event.messages || []) {
      ...
      if (mediaType && !item.key.fromMe) {
        ...
        const media = hasDownloadableMedia(item)
          ? await this.downloadInboundMedia(item, mediaType).catch((error) => {
              this.logger.warn(`WhatsApp Web media download failed: ${error.message}`);
              return null;
            })
          : null;
  ...
  async handleRecoveredHistory(event = {}) {
    const messages = Array.isArray(event.messages) ? event.messages : [];
    if (!this.onRecoveredMessage || !messages.length || !this.recoverySinceAt) return;
    const cutoffMs = Date.parse(this.recoverySinceAt) - 5 * 60 * 1000;
    if (!Number.isFinite(cutoffMs)) return;
    this.diagnostics.recoveredEvents += 1;
    this.diagnostics.recoveredMessages += messages.length;
    let imported = 0;
    for (const item of messages) {
      if (!item?.key || item.key.fromMe) continue;
      const remoteJid = item.key.remoteJid || "";
      if (!isCustomerJid(remoteJid)) continue;
      const messageTimeMs = messageTimestampMs(item);
      if (messageTimeMs && messageTimeMs < cutoffMs) continue;
      const mediaType = messageMediaType(item);
      const text = mediaType ? "" : messageText(item);
      if (!mediaType && !text) continue;
      const sourcePhone = remoteJid.endsWith("@lid") ? "" : normalizePhone(remoteJid.split("@")[0]);
      const source = {
        ...this.messageSource(item, remoteJid, sourcePhone),
        recoveredAfterReconnect: true,
        recoverySinceAt: this.recoverySinceAt,
        historySyncType: String(event.syncType ?? ""),
        historyProgress: event.progress ?? null,
      };
      await this.onRecoveredMessage({
        id: item.key.id || `web_recovered_${Date.now()}`,
        from: toCustomerId(remoteJid),
        text,
        mediaType,
        caption: messageCaption(item),
        ...
        originalTimestamp: messageTimeMs ? new Date(messageTimeMs).toISOString() : "",
      });
      imported += 1;
    }
    this.diagnostics.recoveredImported += imported;
    if (imported) {
      this.logger.log(`WhatsApp Web recovery sync imported ${imported} missed message(s).`);
    }
  }

  async downloadInboundMedia(item = {}, mediaType = "") {
    const baileys = await import("@whiskeysockets/baileys");
    ...
    return {
      ...
      mimeType: messageMediaMimeType(item),
      filename: `${item.key?.id || Date.now()}-${mediaType || "media"}`,
    };
  }
...
function messageTimestampMs(item = {}) {
  ...
  const number = Number(value);
  return Number.isFinite(number) ? (number > 1e12 ? number : number * 1000) : 0;
}

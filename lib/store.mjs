...
const SOURCE_TEXT_MAX_CHARS = 800;
...
export class JsonStore {
  ...
  async getOrCreateCustomer(customerId, patch = {}) {
    if (this.#usesCustomersTable()) {
      return this.#withWriteQueue(async () => {
        const now = new Date().toISOString();
        const businessAccountId = String(patch.businessAccountId || "");
        const existing = await this.adapter.getCustomerRow(customerId, businessAccountId);
        const customer =
          existing ||
          {
            id: customerId,
        ...
        const savedPatch = normalizeCustomerPatch(customer, patch);
        ...
        saved.label = computeCustomerLabel(saved);
        await this.adapter.upsertCustomerRow(saved);
        return saved;
      });
    }
    return this.#withWriteQueue(async () => {
  ...
  async getCustomer(customerId, businessAccountId = "") {
    if (this.#usesCustomersTable()) {
      return this.adapter.getCustomerRow(customerId, businessAccountId);
    }
    const db = await this.#readJson(this.customersPath, { customers: {} });
  ...
  async updateCustomer(customerId, updater, businessAccountId = "") {
    if (this.#usesCustomersTable()) {
      return this.#withWriteQueue(async () => {
        const customer = await this.adapter.getCustomerRow(customerId, businessAccountId);
        ...
        const saved = {
          ...
          ...updater(customer),
          id: customerId,
          businessAccountId: String(businessAccountId || customer.businessAccountId || ""),
          version: Number(customer.version || 0) + 1,
          updatedAt: now,
  ...
  async listCustomers(now = new Date(), businessAccountId = "") {
    if (this.#usesCustomersTable()) {
      const customers = await this.adapter.listCustomerRows(businessAccountId);
      return customers
  ...
  async exportCustomerData(customerId, businessAccountId = "") {
    ...
    const activeCustomer = await this.getCustomer(customerId, businessAccountId);
  ...
  async deleteCustomer(customerId, reason = "Manual admin deletion", now = new Date(), businessAccountId = "") {
    if (this.#usesCustomersTable()) {
      ...
      await this.adapter.deleteCustomerRow(customerId, businessAccountId);
      if (this.#usesOrdersTable()) {
        await this.adapter.deleteOrderRowsForCustomer(customerId, businessAccountId);
      } else {
  ...
  async addOrder(order) {
    const now = new Date().toISOString();
    const saved = {
      id: order.id || `ord_${Date.now()}_${Math.floor(Math.random() * 1000000)}`,
      createdAt: now,
      updatedAt: now,
      ...order,
    ...
    if (this.#usesOrdersTable()) {
      await this.adapter.addOrderRow(saved);
    } else {
  ...
  async listOrders(businessAccountId = "") {
    if (this.#usesOrdersTable()) {
      return this.adapter.listOrderRows(businessAccountId);
    }
    const db = await this.#readJson(this.ordersPath, { orders: [] });
  ...
  async updateOrderStatus(orderId, status, actorId) {
    ...
    const db = this.#usesOrdersTable() ? null : await this.#readJson(this.ordersPath, { orders: [] });
    const order = this.#usesOrdersTable() ? await this.adapter.getOrderRow(orderId) : db.orders.find((item) => item.id === orderId);
    ...
    if (this.#usesOrdersTable()) {
      await this.adapter.updateOrderRow(order);
    } else {
      await this.#writeJson(this.ordersPath, db);
    }
    return order;
  ...
  async updateOrdersForStockArrival(productId, businessAccountId = "", actorId = "business_admin") {
    const orders = await this.listOrders(businessAccountId);
    const now = new Date().toISOString();
    const changed = [];
    ...
    if (this.#usesOrdersTable()) {
      for (const order of changed) await this.adapter.updateOrderRow(order);
    } else {
      const db = await this.#readJson(this.ordersPath, { orders: [] });
      const changedById = new Map(mapped.map((order) => [order.id, order]));
      db.orders = db.orders.map((order) => changedById.get(order.id) || order);
      await this.#writeJson(this.ordersPath, db);
    }
    return changed;
  }
  ...
  async appendOutbox(message) {
    if (this.#usesOutboxTable()) {
      const [saved] = await this.appendOutboxMany([message]);
      return saved;
    }
  ...
  async claimProcessedMessage(messageId, businessAccountId = "", metadata = {}) {
    ...
    if (this.#usesProcessedMessagesTable()) {
      return this.adapter.claimProcessedMessageRow(id, businessAccountId, metadata, this.processedMessageRetentionMs);
    }
    return this.#withWriteQueue(async () => {
  ...
  async getProcessedMessage(messageId, businessAccountId = "") {
    ...
    if (!id) return null;
    if (this.#usesProcessedMessagesTable()) {
      return this.adapter.getProcessedMessageRow(id, businessAccountId);
    }
    const db = await this.#readJson(this.processedMessagesPath, { messages: {} });
  ...
  async savePendingBuffer(key, buffer = {}) {
    ...
    if (this.#usesPendingBuffersTable()) {
      return this.#withWriteQueue(async () => {
        const existing = await this.adapter.getPendingBufferRow(id) || {};
        const now = new Date().toISOString();
        return this.adapter.upsertPendingBufferRow({
          ...existing,
          ...buffer,
          key: id,
  ...
  async updatePendingBuffer(key, updater) {
    ...
    if (this.#usesPendingBuffersTable()) {
      return this.#withWriteQueue(async () => {
        ...
        if (!next) {
          await this.adapter.deletePendingBufferRow(id);
          return null;
        }
        const now = new Date().toISOString();
        return this.adapter.upsertPendingBufferRow({
  ...
  async getPendingBuffer(key) {
    ...
    if (!id) return null;
    if (this.#usesPendingBuffersTable()) {
      return this.adapter.getPendingBufferRow(id);
    }
    const db = await this.#readJson(this.pendingBuffersPath, { buffers: {} });
  ...
  async listPendingBuffers(businessAccountId = "") {
    if (this.#usesPendingBuffersTable()) {
      return this.adapter.listPendingBufferRows(businessAccountId);
    }
    const db = await this.#readJson(this.pendingBuffersPath, { buffers: {} });
  ...
  async deletePendingBuffer(key) {
    ...
    if (!id) return false;
    if (this.#usesPendingBuffersTable()) {
      return this.#withWriteQueue(() => this.adapter.deletePendingBufferRow(id));
    }
    return this.#withWriteQueue(async () => {
      const db = await this.#readJson(this.pendingBuffersPath, { buffers: {} });
  ...
  async hasOutboxMessageId(messageId, businessAccountId = "") {
    ...
    if (!id) return false;
    if (this.#usesOutboxTable()) {
      return this.adapter.hasOutboxMessageId(id, businessAccountId);
    }
    const db = await this.#readJson(this.outboxPath, { messages: [] });
  ...
  async appendOutboxMany(messages) {
    ...
    if (this.#usesOutboxTable()) {
      ...
      const saved = messages.map((message, index) => ({
        id: message.id || `msg_${now}_${index + 1}_${Math.floor(Math.random() * 1000000)}`,
        createdAt: message.createdAt || new Date().toISOString(),
        ...compactStoredMessage(message),
      }));
      return this.adapter.appendOutboxMessages(saved, this.outboxRetentionMs);
    }
    const db = await this.#readJson(this.outboxPath, { messages: [] });
    const now = Date.now();
    const saved = messages.map((message, index) => ({
  ...
  async listOutbox(businessAccountId = "") {
    if (this.#usesOutboxTable()) {
      return this.adapter.listOutboxMessages(businessAccountId);
    }
    const db = await this.#readJson(this.outboxPath, { messages: [] });
  ...
  async deleteConversationMessages(customerId, businessAccountId = "") {
    ...
    if (this.#usesOutboxTable()) {
      return this.adapter.deleteConversationMessages(id, businessAccountId);
    }
    const db = await this.#readJson(this.outboxPath, { messages: [] });
  ...
  async appendAuditLog(event) {
    if (this.#usesAuditEventsTable()) {
      ...
      return this.adapter.appendAuditEvent(saved, this.auditLogMaxRows);
    }
  ...
  async listAuditLog() {
    if (this.#usesAuditEventsTable()) {
      return this.adapter.listAuditEvents();
    }
    const db = await this.#readJson(this.auditPath, { events: [] });
    return db.events;
  }

  async purgeAccountData(businessAccountId = "") {
    const accountId = String(businessAccountId || "").trim();
    if (!accountId) throw new Error("Business account ID is required.");
    const summary = {
      customers: 0,
      deletedCustomers: 0,
      orders: 0,
    ...
    if (this.adapter?.deleteCustomerRowsForAccount) {
      summary.customers += await this.adapter.deleteCustomerRowsForAccount(accountId);
    }
    if (this.adapter?.deleteOrderRowsForAccount) {
      summary.orders += await this.adapter.deleteOrderRowsForAccount(accountId);
    }
    if (this.adapter?.deleteOutboxForAccount) {
      summary.outbox += await this.adapter.deleteOutboxForAccount(accountId);
    }
    if (this.adapter?.deleteProcessedMessageRowsForAccount) {
      summary.processedMessages += await this.adapter.deleteProcessedMessageRowsForAccount(accountId);
    }
    if (this.adapter?.deletePendingBufferRowsForAccount) {
      summary.pendingBuffers += await this.adapter.deletePendingBufferRowsForAccount(accountId);
    }
    if (this.adapter?.deleteAuditEventsForAccount) {
      summary.auditEvents += await this.adapter.deleteAuditEventsForAccount(accountId);
    }

    await this.#purgeJsonCustomerData(accountId, summary);
    await this.#purgeJsonOutboxData(accountId, summary);
    await this.#purgeJsonOrderData(accountId, summary);
    await this.#purgeJsonComplaintData(accountId, summary);
    await this.#purgeJsonProcessedMessages(accountId, summary);
    await this.#purgeJsonPendingBuffers(accountId, summary);
    await this.#purgeJsonAuditEvents(accountId, summary);
    ...
    if (this.#usesCustomersTable()) {
      ...
      for (const customer of customers) {
        ...
        if (ageDays >= 11 && hasNoReplyAfterFirstChat && hasNoOrder) {
          ...
          deletedDb.customers.push(deletedCustomer);
          await this.adapter.deleteCustomerRow(customer.id, customer.businessAccountId || "");
        }
      }
      if (deleted.length) await this.#writeJson(this.deletedCustomersPath, deletedDb);
      return deleted;
    }
    const db = await this.#readJson(this.customersPath, { customers: {} });
    const deletedDb = await this.#readJson(this.deletedCustomersPath, { customers: [] });
  ...
  async #readJsonIfExists(filePath) {
    if (this.adapter?.readJsonIfExists) {
      const existing = await this.adapter.readJsonIfExists(filePath);
  ...
  async #updateProcessedMessage(messageId, businessAccountId, patch) {
    ...
    if (!id) return null;
    if (this.#usesProcessedMessagesTable()) {
      return this.adapter.updateProcessedMessageRow(id, businessAccountId, patch, this.processedMessageRetentionMs);
    }
    return this.#withWriteQueue(async () => {
      const db = await this.#readJson(this.processedMessagesPath, { messages: {} });
      const key = processedMessageKey(id, businessAccountId);
  ...
  #usesOutboxTable() {
    return Boolean(
      this.adapter?.outboxTableEnabled &&
      this.adapter?.appendOutboxMessages &&
      this.adapter?.listOutboxMessages &&
      this.adapter?.hasOutboxMessageId &&
      this.adapter?.deleteConversationMessages
    );
  }

  #usesCustomersTable() {
    return Boolean(
      this.adapter?.customersTableEnabled &&
      this.adapter?.getCustomerRow &&
      this.adapter?.upsertCustomerRow &&
      this.adapter?.listCustomerRows &&
      this.adapter?.deleteCustomerRow
    );
  }

  #usesOrdersTable() {
    return Boolean(
      this.adapter?.ordersTableEnabled &&
      this.adapter?.addOrderRow &&
      this.adapter?.listOrderRows &&
      this.adapter?.getOrderRow &&
      this.adapter?.updateOrderRow &&
      this.adapter?.deleteOrderRowsForCustomer
    );
  }

  #usesProcessedMessagesTable() {
    return Boolean(
      this.adapter?.processedMessagesTableEnabled &&
      this.adapter?.claimProcessedMessageRow &&
      this.adapter?.updateProcessedMessageRow &&
      this.adapter?.getProcessedMessageRow
    );
  }

  #usesPendingBuffersTable() {
    return Boolean(
      this.adapter?.pendingBuffersTableEnabled &&
      this.adapter?.upsertPendingBufferRow &&
      this.adapter?.getPendingBufferRow &&
      this.adapter?.listPendingBufferRows &&
      this.adapter?.deletePendingBufferRow
    );
  }

  #usesAuditEventsTable() {
    return Boolean(
      this.adapter?.auditEventsTableEnabled &&
      this.adapter?.appendAuditEvent &&
      this.adapter?.listAuditEvents
    );
  }
  ...
  async #purgeJsonCustomerData(accountId, summary) {
    const db = await this.#readJsonIfExists(this.customersPath);
    if (db?.customers && typeof db.customers === "object" && !Array.isArray(db.customers)) {
      const entries = Object.entries(db.customers);
      db.customers = Object.fromEntries(entries.filter(([key, customer]) =>
        !belongsToBusiness(customer, accountId) && !String(key).startsWith(`${accountId}::`)
      ));
      const removed = entries.length - Object.keys(db.customers).length;
      if (removed) {
        summary.customers += removed;
        await this.#writeJson(this.customersPath, db);
      }
    }

    const deletedDb = await this.#readJsonIfExists(this.deletedCustomersPath);
    if (Array.isArray(deletedDb?.customers)) {
      const before = deletedDb.customers.length;
      deletedDb.customers = deletedDb.customers.filter((customer) => !belongsToBusiness(customer, accountId));
      const removed = before - deletedDb.customers.length;
      if (removed) {
        summary.deletedCustomers += removed;
        await this.#writeJson(this.deletedCustomersPath, deletedDb);
      }
    }
  }

  async #purgeJsonOutboxData(accountId, summary) {
    const db = await this.#readJsonIfExists(this.outboxPath);
    if (!Array.isArray(db?.messages)) return;
    const before = db.messages.length;
    db.messages = db.messages.filter((message) => !belongsToBusiness(message, accountId));
    const removed = before - db.messages.length;
    if (removed) {
      summary.outbox += removed;
      await this.#writeJson(this.outboxPath, db);
    }
  }

  async #purgeJsonOrderData(accountId, summary) {
    const db = await this.#readJsonIfExists(this.ordersPath);
    if (!Array.isArray(db?.orders)) return;
    const before = db.orders.length;
    db.orders = db.orders.filter((order) => !belongsToBusiness(order, accountId));
    const removed = before - db.orders.length;
    if (removed) {
      summary.orders += removed;
      await this.#writeJson(this.ordersPath, db);
    }
  }

  async #purgeJsonComplaintData(accountId, summary) {
    const db = await this.#readJsonIfExists(this.complaintsPath);
    if (!Array.isArray(db?.cases)) return;
    const before = db.cases.length;
    db.cases = db.cases.filter((item) => !belongsToBusiness(item, accountId));
    const removed = before - db.cases.length;
    if (removed) {
      summary.complaints += removed;
      await this.#writeJson(this.complaintsPath, db);
    }
  }

  async #purgeJsonProcessedMessages(accountId, summary) {
    const db = await this.#readJsonIfExists(this.processedMessagesPath);
    if (!db?.messages || typeof db.messages !== "object" || Array.isArray(db.messages)) return;
    const entries = Object.entries(db.messages);
    db.messages = Object.fromEntries(entries.filter(([key, message]) =>
      !belongsToBusiness(message, accountId) && !String(key).startsWith(`${accountId}::`)
    ));
    const removed = entries.length - Object.keys(db.messages).length;
    if (removed) {
      summary.processedMessages += removed;
      await this.#writeJson(this.processedMessagesPath, db);
    }
  }

  async #purgeJsonPendingBuffers(accountId, summary) {
    const db = await this.#readJsonIfExists(this.pendingBuffersPath);
    if (!db?.buffers || typeof db.buffers !== "object" || Array.isArray(db.buffers)) return;
    const entries = Object.entries(db.buffers);
    db.buffers = Object.fromEntries(entries.filter(([key, buffer]) =>
      !belongsToBusiness(buffer, accountId) && !String(key).startsWith(`${accountId}:`)
    ));
    const removed = entries.length - Object.keys(db.buffers).length;
    if (removed) {
      summary.pendingBuffers += removed;
      await this.#writeJson(this.pendingBuffersPath, db);
    }
  }

  async #purgeJsonAuditEvents(accountId, summary) {
    const db = await this.#readJsonIfExists(this.auditPath);
    if (!Array.isArray(db?.events)) return;
    const before = db.events.length;
    db.events = db.events.filter((event) => {
      const actor = String(event?.actor || "");
      return (
        !belongsToBusiness(event, accountId) &&
        event?.accountId !== accountId &&
        event?.result !== accountId &&
        actor !== `admin:${accountId}` &&
        actor !== `business_admin:${accountId}`
      );
    });
    const removed = before - db.events.length;
    if (removed) {
      summary.auditEvents += removed;
      await this.#writeJson(this.auditPath, db);
    }
  }
}
...
function compactStoredMessage(message = {}) {
  const compacted = { ...message };
  if (compacted.source) compacted.source = compactMessageSource(compacted.source);
  if (compacted.meta?.source) {
    compacted.meta = {
      ...compacted.meta,
      source: compactMessageSource(compacted.meta.source),
    };
  }
  return compacted;
}

function normalizeCustomerPatch(customer = {}, patch = {}) {
  const savedPatch = { ...patch };
  delete savedPatch.recordInbound;
  if (savedPatch.source) {
    const incomingSource = compactMessageSource(savedPatch.source);
    const existingSource = compactMessageSource(customer.source || {});
    savedPatch.source = mergeMessageSources(existingSource, incomingSource);
    if (patch.recordInbound) savedPatch.lastInboundSource = incomingSource;
    const existingLeadSource = compactMessageSource(customer.leadSource || {});
    if (hasLeadSourceClues(existingLeadSource)) {
      savedPatch.leadSource = existingLeadSource;
    } else if (hasLeadSourceClues(incomingSource)) {
      savedPatch.leadSource = incomingSource;
    }
  }
  return savedPatch;
}

function compactMessageSource(source = {}) {
  const compacted = { ...source };
  for (const key of ["adBody", "referralBody", "body", "rawBody", "text"]) {
    if (typeof compacted[key] === "string") compacted[key] = truncateSourceText(compacted[key]);
  }
  return compacted;
}

function mergeMessageSources(existing = {}, incoming = {}) {
  ...
  return compactMessageSource(merged);
}

function hasLeadSourceClues(source = {}) {
  return Boolean(
    source.productId ||
      source.adId ||
      source.adTitle ||
...
function truncateSourceText(value = "") {
  ...
  return `${text.slice(0, SOURCE_TEXT_MAX_CHARS).trimEnd()}...`;
}
...
function followupDueAt(firstSeenAt, item) {
  ...
  if (item.customSchedule) {
    const firstSeenLocal = zonedDateParts(anchorAt);

...
test("structured FAQ sales expectation treats buleh as package interest", async () => {
  const customer = {
  ...
  assert.equal(classifyFaqSalesPromptResponse(customer, "buleh"), "interested");
  assert.equal(plan.customerPatch.awaitingPackageBInterest, false);
...
test("sales reply preserves package-interest state so ok can proceed to order form", async () => {
  ...
  assert.equal(classifyFaqSalesPromptResponse(acceptedCustomer, "ok"), "interested");
...
test("ok after the FAQ sales question sends the order form without GPT", async () => {
  const customer = {
  ...
  assert.equal(classifyFaqSalesPromptResponse(customer, "ok"), "interested");
  assert.equal(plan.customerPatch.awaitingPackageBInterest, false);
  assert.equal(plan.customerPatch.pendingOrder.productId, product.id);
  assert.match(plan.messages[0].body, /Full name/);
});

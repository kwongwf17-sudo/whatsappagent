...
test("new customer sending only product name receives that product opening flow", async () => {
  const plan = await buildConversationPlan({
    ...
    productFactMatch: {
      factId: "wrong_early_fact",
      reply: "This should not be sent before the opening flow.",
    },
  });
...
test("existing customer sending product name receives opening flow instead of image knowledge", async () => {
  const plan = await buildConversationPlan({
    ...
    source: { productId: ilsoProduct.id },
    productFactMatch: {
      factId: "wrong_image_chunk",
      reply: "Promotional sales image raw summary should not be sent.",
    },
  });
...
test("product name inside a longer message does not trigger opening flow", async () => {
  ...
  const plan = await buildConversationPlan({
    ...
    source: { productId: ilsoProduct.id },
    productFactMatch: {
      factId: "direct_product_answer",
      reply: "This should be treated as a normal product question.",
    },
  });

  assert.equal(plan.messages.length, 2);
  assert.equal(plan.messages[0].body, "This should be treated as a normal product question.");
});
...
test("approved image chunks answer product detail questions", async () => {
  ...
  assert.equal(plan.messages[0].body, "Isinya 150ml ya.");
  assert.equal(plan.handoffRequired, false);
});

test("blackhead feature questions answer from approved image chunks without OpenAI", async () => {
  const cases = [
    ["Suction kuat tak?", /60KPA strong suction/i],
    ["Ada berapa mode?", /3 suction modes: Normal, Intermediate dan Strong/i],
    ["Buleh recharge?", /USB-C rechargeable/i],
  ];

  for (const [customerMessage, expected] of cases) {
    const plan = await buildConversationPlan({
      catalog,
    ...
    assert.match(plan.messages[0].body, expected, customerMessage);
    assert.equal(plan.messages[1].body, blackheadSalesPrompt, customerMessage);
    assert.equal(plan.handoffRequired, false, customerMessage);
  }
});

test("retrieved product facts answer when local fact rules do not match", async () => {
  const plan = await buildConversationPlan({
    catalog,
  ...
  assert.equal(plan.customerPatch.lastProductFactId, "fact_volume");
  assert.equal(plan.handoffRequired, false);
});

test("retrieved image chunk can use generated customer-facing reply", async () => {
  const plan = await buildConversationPlan({
    catalog,
    customer: { id: "customer_generated_chunk_reply", productId: ilsoProduct.id },
    customerMessage: "produk ni fungsi apa",
    productFactMatch: {
      factId: "image_function",
  ...
  assert.equal(plan.customerPatch.lastProductFactId, "image_function");
  assert.equal(plan.handoffRequired, false);
});
...
test("feature image chunks send polished product answers instead of raw source text", async () => {
  const chunkCatalog = structuredClone(catalog);
  ...
  assert.equal(plan.handoffRequired, false);
  assert.match(plan.messages[0].body, /USB-C rechargeable/i);

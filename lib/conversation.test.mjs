...
test("sales reply state suppresses repeated same intent after max sends", async () => {
  const salesReplyLibrary = {
    sales_replies: [
      {
  ...
  const repeatedPlan = await buildConversationPlan({
    ...
    customer: {
      ...
      productId: product.id,
      salesReplyState: firstPlan.customerPatch.salesReplyState,
    },
    customerMessage: "Fikir dulu",
  });
  ...
  assert.equal(repeatedPlan.customerPatch.lastSalesReplySuppressedIntent, "thinking_first");
  assert.equal(repeatedPlan.customerPatch.lastSalesReplySuppressedReason, "max_sends_reached");
});

test("sales reply cooldown suppresses repeated same intent before cooldown expires", async () => {
  const salesReplyLibrary = {
    sales_replies: [
      {
  ...
  assert.equal(repeatedPlan.messages.length, 0);
  assert.equal(repeatedPlan.customerPatch.lastSalesReplySuppressedReason, "cooldown_active");
});

...
test("sales reply preserves package-interest state so ok can proceed to order form", async () => {
  const approvedReply = "Bh skrg kami buleh hold promo dlu, skrg nda byr apa2, jadi bila delivery baru byr Runner saja.\nMana 1 package kita rasa minat nak hold promo?";
  ...
  const firstPlan = await buildConversationPlan({
    ...
    salesReplyMatch: {
      salesReplyId: "sales_payday_hold",
      salesIntent: "payday_only_pay",
      objectionType: "Payday / budget",
      intent: "Customer has budget issue but may hold promo and pay later.",
      approvedReply,
      afterReply: "WAIT_FOR_CUSTOMER",
      expectsPackageInterest: true,
    },
  });

  assert.equal(firstPlan.messages[0].body, approvedReply);

...
test("Brunei Kpg address details extract from separate lines", async () => {
  const plan = await buildConversationPlan({
    catalog,
    customer: { id: "customer_kpg_multiline_order", productId: product.id },
    customerMessage: "wong\nKpg Buaya, 120002\n7335901",
  });

  assert.equal(plan.order, undefined);
  assert.equal(plan.handoffRequired, false);
  assert.equal(plan.customerPatch.pendingOrder.draft.name, "wong");
  assert.equal(plan.customerPatch.pendingOrder.draft.phone, "7335901");
  assert.equal(plan.customerPatch.pendingOrder.draft.address, "Kpg Buaya, 120002");
  assert.match(plan.messages[0].body, /tinggal pilih order option/i);
  assert.doesNotMatch(plan.messages[0].body, /Full address/i);
});

test("Brunei Kpg address details extract from one line", async () => {
  const plan = await buildConversationPlan({
    catalog,
    customer: { id: "customer_kpg_single_line_order", productId: product.id },
    customerMessage: "wong Kpg Buaya 120002 7335901",
  });
...
test("package price question uses RAG and does not begin order collection", async () => {
  const plan = await buildConversationPlan({
    catalog,
...
test("conversation plan exposes routing decision metadata", async () => {
  ...
  assert.equal(plan.routingDecision.rule, "fallback_knowledge");
  ...
  assert.ok(plan.routingDecision.trace.some((item) => item.rule === "fallback_knowledge" && item.matched));
});
...
test("generic order intent starts blank order option instead of preselecting a package", async () => {
  const plan = await buildConversationPlan({
    catalog,
    customer: { id: "customer_generic_order_blank_option", productId: product.id, awaitingPackageBInterest: true },
    customerMessage: "nak order",
    routeClassification: {
      messageType: "purchase_intent",
      confidence: "high",
      primaryIntent: "",
  ...
  assert.match(plan.messages[0].body, /Order option\s*:\s*$/i);
  assert.doesNotMatch(plan.messages[0].body, /Order option\s*:\s*Package A/i);
});

test("contextual purchase intent in awaiting package interest starts order collection", async () => {
  const plan = await buildConversationPlan({
    catalog,
    customer: {
      id: "customer_contextual_saya_mau",
      productId: product.id,
...
test("misspelled package option replies map to configured order option", async () => {
  for (const customerMessage of ["pakage b", "packed b", "pack b"]) {
    const plan = await buildConversationPlan({
      catalog,
      customer: { id: `customer_${customerMessage.replace(/\s+/g, "_")}`, productId: product.id },
      customerMessage,
      routeClassification: {
        messageType: "purchase_intent",
        confidence: "high",
    ...
    assert.equal(plan.customerPatch.pendingOrder.productId, product.id, customerMessage);
    assert.equal(plan.customerPatch.pendingOrder.draft.orderOptionName, "Package B", customerMessage);
    assert.match(plan.messages[0].body, /Full name/i, customerMessage);
  }
});

test("free-form order submission recognizes misspelled package option", async () => {
  const parsed = extractOrderDetails(
    [
      "OK saya nak ambil Pakage B",
      "Hjh Aisah Hj Taha",
      "No 47 Spg 42, Jln Pengkalan Pinang, Kg Penanjong Tutong, TA 2741",
      "PAKAGE B",
      "TEL : 8157329",
    ].join("\n"),
    product
  ...
  assert.equal(parsed.orderOptionName, "Package B");
  assert.equal(parsed.name, "Hjh Aisah Hj Taha");
  assert.equal(parsed.phone, "8157329");
  assert.equal(parsed.isComplete, true);
});

test("free-form order submission recognizes Brunei RPN address cue", async () => {
  const parsed = extractOrderDetails("Zul RPN Kampung Rimba Blok C No 7 8987654", product);

  assert.equal(parsed.name, "Zul");
  assert.equal(parsed.phone, "8987654");
  assert.equal(parsed.address, "RPN Kampung Rimba Blok C No 7");
});

test("free-form order submission handles dash separated package details", async () => {
  const parsed = extractOrderDetails("Package B - Aisyah - Kpg Lumut 120003 - 8112233", product);

  assert.equal(parsed.orderOptionName, "Package B");
  assert.equal(parsed.name, "Aisyah");
  assert.equal(parsed.phone, "8112233");
  assert.equal(parsed.address, "Kpg Lumut 120003");
});

test("free-form order submission handles inline Malay labelled details", async () => {
  const parsed = extractOrderDetails("nak package A, nama Sarah, alamat Kg Rimba, no 7335901", product);

  assert.equal(parsed.orderOptionName, "Package A");
  assert.equal(parsed.name, "Sarah");
  assert.equal(parsed.phone, "7335901");
  assert.equal(parsed.address, "Kg Rimba");
});

test("free-form order submission handles comma name phone and bare package line", async () => {
  const parsed = extractOrderDetails(["Khad ,7342742", "6, spg 124 Kg madang", "A"].join("\n"), product);

  assert.equal(parsed.orderOptionName, "Package A");
  assert.equal(parsed.name, "Khad");
  assert.equal(parsed.phone, "7342742");
  assert.equal(parsed.address, "6, spg 124 Kg madang");
  assert.equal(parsed.isComplete, true);
});
...
test("new ad price question receives only opening flow without same-turn RAG answer", async () => {
  const plan = await buildConversationPlan({
    catalog,
...
test("new customer product name plus price question receives only opening flow without same-turn RAG answer", async () => {
  const cases = [
  ...
  for (const customerMessage of cases) {
    ...
    assert.equal(plan.handoffRequired, false, customerMessage);
    assert.deepEqual(plan.messages, ilsoProduct.opening_flow, customerMessage);
    assert.doesNotMatch(plan.messages.map((message) => message.body || "").join("\n"), /Single Unit: B\$39/, customerMessage);
  }
});
...
test("new ad sales objection receives only opening flow without same-turn sales reply", async () => {
  ...
  assert.deepEqual(plan.messages, product.opening_flow);
  assert.equal(plan.customerPatch.lastSalesReplyIntent, undefined);
});
...
test("uncertain general first message asks which product before answering FAQs", async () => {
  const plan = await buildConversationPlan({
    catalog,
    customer: {
      id: "new_customer_uncertain_general_question",
      productId: "",
      inboundCount: 0,
    },
    customerMessage: "Area mana di brunei ..n kedai nya di mana",
    source: { transport: "web" },
    isFirstEligibleInbound: true,
    routeClassification: {
      messageType: "general_faq",
...
test("short charge question without context uses classifier clarification", async () => {
  const plan = await buildConversationPlan({
    catalog,
    faqLibrary,
    ...
    routeClassification: {
      ...
      confidence: "low",
      reason: "Charge reference is ambiguous without recent context.",
    },
  });

  assert.match(plan.messages[0].body, /clarify/i);
  assert.equal(plan.handoffRequired, false);
});

test("short unclear question uses classifier clarification instead of handoff", async () => {
  const plan = await buildConversationPlan({
    catalog,
    faqLibrary,
    ...
    routeClassification: {
      ...
      humanSupportRequired: false,
      confidence: "low",
      reason: "Ambiguous option reference.",
    },
  });
...
test("classifier clarification flag asks one clarification instead of handoff", async () => {
  const plan = await buildConversationPlan({
    catalog,
    faqLibrary,
    customer: { id: "customer_classifier_unclear", productId: product.id },
    customerMessage: "yang atu",
    routeClassification: {
      messageType: "unknown",
      clarificationRequired: true,
      humanSupportRequired: false,
  ...
  assert.match(plan.messages[0].body, /clarify|maksudkan/i);
  assert.equal(plan.handoffRequired, false);
  assert.equal(plan.customerPatch.unclearClarificationCount, 1);
});

test("repeated classifier clarification hands off after one clarification", async () => {
  const plan = await buildConversationPlan({
    catalog,
    faqLibrary,
    customer: {
      id: "customer_classifier_unclear_again",
      productId: product.id,
      unclearClarificationCount: 1,
      lastClarificationAt: new Date().toISOString(),
    },
    customerMessage: "atu tadi",
    routeClassification: {
      messageType: "unknown",
      clarificationRequired: true,
      humanSupportRequired: false,
  ...
  assert.equal(plan.handoffRequired, true);
  assert.equal(plan.customerPatch.handoffStatus, "human_required");
  assert.equal(plan.customerPatch.handoffReason, "Customer message remains unclear after clarification.");
});
...
test("product order option price question requires RAG instead of local configured options", async () => {
  const plan = await buildConversationPlan({
    catalog,
    customer: { id: "customer_ilso_price", productId: ilsoProduct.id },
    customerMessage: "harganya satu $39 or $55?",
    ragAnswer: {
      reply: "Single Unit: B$39. Special Combo: B$55.",
      replyType: "faq",
      handoffRequired: false,
      allowProductSpecific: true,
    },
...
test("product measurement question without RAG does not use local price fallback", async () => {
  const plan = await buildConversationPlan({
    catalog,
    customer: { id: "customer_ilso_measurement", productId: ilsoProduct.id },
    customerMessage: "1 botol berapa gram barat nya n boleh pakai utk berapa pokok",
  });
...
test("FAQ sales prompt frequency can send every second answered question", async () => {
  ...
  assert.equal(secondPlan.customerPatch.expectedNextAction.type, "package_interest_confirmation");
  assert.equal(secondPlan.customerPatch.expectedNextAction.source, "faq_sales_followup");
  assert.equal(secondPlan.customerPatch.expectedNextAction.productId, customProduct.id);
  assert.equal(secondPlan.customerPatch.faqSalesPromptCounts[customProduct.id], 2);
});

test("structured FAQ sales expectation starts order only when classifier confirms package interest", async () => {
  const customer = {
    id: "customer_structured_accepts_package_b",
    productId: product.id,
  ...
  assert.equal(classifyFaqSalesPromptResponse(customer, "buleh"), "");
  assert.equal(plan.customerPatch.awaitingPackageBInterest, false);
  assert.equal(plan.customerPatch.expectedNextAction, null);
  assert.equal(plan.customerPatch.pendingOrder.productId, product.id);
  assert.match(plan.messages[0].body, /Full name/);
});

test("expired structured FAQ sales expectation falls back to normal routing", async () => {
  const customer = {
    id: "customer_expired_structured_package_interest",
    productId: product.id,
    awaitingPackageBInterest: false,
    expectedNextAction: {
      type: "package_interest_confirmation",
      source: "faq_sales_followup",
      productId: product.id,
      setAt: new Date(Date.now() - 120_000).toISOString(),
      expiresAt: new Date(Date.now() - 60_000).toISOString(),
    },
  };
...
test("product approved FAQ exact examples can be excluded from local matching", () => {
  const faq = findApprovedFaqExactMatch(catalog, product, "Macam mana guna Blackhead Remover?", {
    faqLibrary,
...
test("approved FAQ primary intent match uses classifier FAQ id before wording/RAG fallback", () => {
  const match = findApprovedFaqPrimaryIntentMatch(catalog, product, {
    messageType: "general_faq",
    primaryIntent: "general_pay_end_month",
    confidence: "high",
  }, { faqLibrary });

  assert.equal(match?.id, "general_pay_end_month");
  assert.equal(match?.scope, "general");
});

test("approved FAQ primary intent match keeps general and product FAQ scopes separate", () => {
  const productWithFaq = {
    ...product,
    approved_faqs: [
      {
        id: "product_specific_payday",
        topic: "Product payday",
        example_questions: ["Can pay later?"],
        approved_reply: "Product-specific later payment reply",
        active: true,
      },
    ],
  };
  const match = findApprovedFaqPrimaryIntentMatch(catalog, productWithFaq, {
    messageType: "general_faq",
    primaryIntent: "product_specific_payday",
    confidence: "high",
  }, { faqLibrary });
...
test("pending order reference uses merged saved Kpg address instead of repeating full form", async () => {
  const plan = await buildConversationPlan({
    catalog,
    customer: {
      id: "customer_pending_order_kpg_above",
      productId: product.id,
      pendingOrder: {
        productId: product.id,
        draft: {
          name: "wong",
          address: "Kpg Buaya, 120002",
          phone: "7335901",
          quantity: 1,
        },
      },
    },
    customerMessage: "sya dah bagi full address kat atas",
    routeClassification: {
      messageType: "unknown",
      confidence: "medium",
    },
...
test("product FAQ answer can use contextual sales follow-up instead of generic prompt", async () => {
  const plan = await buildConversationPlan({
    catalog,
    customer: { id: "customer_product_faq_contextual_followup", productId: product.id },
    customerMessage: "Until when promo?",
    ragAnswer: {
      reply: "Promo sampai esok saja ya.",
      replyType: "faq",
      handoffRequired: false,
      allowProductSpecific: true,
    },
    knowledgeSalesFollowup: "Kita tunggu gaji kah?",
  });

  assert.equal(plan.messages[0].body, "Promo sampai esok saja ya.");
  assert.equal(plan.messages[1].body, "Kita tunggu gaji kah?");
});

test("product FAQ answer can suppress sales follow-up when selector says none", async () => {
  const plan = await buildConversationPlan({
    catalog,
    customer: { id: "customer_product_faq_no_followup", productId: product.id },
    customerMessage: "not interested, but COD?",
    ragAnswer: {
      reply: "Boleh, COD to all Brunei.",
      replyType: "faq",
      handoffRequired: false,
...
test("sales reply examples match natural hesitation variants before FAQ follow-up", async () => {
  ...
  assert.equal(plan.customerPatch.awaitingPackageBInterest, undefined);
  assert.equal(plan.customerPatch.expectedNextAction, undefined);
});

test("sales reply preserves package-interest state so ok can proceed to order form", async () => {
  const salesReplyLibrary = {
    sales_replies: [
      {
        id: "sales_payday_hold",
        scope: "business",
        productId: "",
        sales_intent: "payday_only_pay",
        objection_type: "Payday / budget",
        intent: "Customer has budget issue but may hold promo and pay later.",
        example_messages: ["duit tak cukup skrg"],
        approved_reply: "Bh skrg kami buleh hold promo dlu, skrg nda byr apa2, jadi bila delivery baru byr Runner saja.\nMana 1 package kita rasa minat nak hold promo?",
        active: true,
      },
    ],
  };
  ...
  const firstPlan = await buildConversationPlan({
    ...
    salesReplyLibrary,
    customer: {
      id: "customer_budget_then_ok",
      productId: product.id,
      awaitingPackageBInterest: true,
    },
    customerMessage: "skrg duit tak cukup",
  });

  assert.equal(firstPlan.messages[0].body, salesReplyLibrary.sales_replies[0].approved_reply);
  assert.equal(firstPlan.customerPatch.awaitingPackageBInterest, true);
  assert.equal(firstPlan.customerPatch.expectedNextAction.type, "package_interest_confirmation");

  const acceptedCustomer = {
    id: "customer_budget_then_ok",
    productId: product.id,
    awaitingPackageBInterest: firstPlan.customerPatch.awaitingPackageBInterest,
    expectedNextAction: firstPlan.customerPatch.expectedNextAction,
  ...
  assert.equal(classifyFaqSalesPromptResponse(acceptedCustomer, "ok"), "");
  assert.equal(secondPlan.customerPatch.awaitingPackageBInterest, false);
  assert.equal(secondPlan.customerPatch.pendingOrder.productId, product.id);
  assert.match(secondPlan.messages[0].body, /Full name/);
});
...
test("sales primary intent match uses classifier intent when one active template matches", () => {
  const library = {
    sales_replies: [
      {
        ...
        intent: "Customer is thinking first.",
        approved_reply: "No worries kita.",
        active: true,
      },
      {
        ...
        intent: "Customer is not interested.",
        approved_reply: "Terima kasih kita.",
        active: true,
      },
    ],
  };

  const match = findSalesReplyPrimaryIntentMatch(catalog, product, {
    messageType: "sales_reply",
    primaryIntent: "thinking_first",
    confidence: "high",
  }, { salesReplyLibrary: library });

  assert.equal(match?.id, "sales_thinking_first");
});

test("sales primary intent match falls back when multiple templates share one intent", () => {
  const library = {
    sales_replies: [
      {
        id: "sales_thinking_first_a",
        sales_intent: "thinking_first",
        approved_reply: "No worries kita.",
        active: true,
      },
      {
        id: "sales_thinking_first_b",
        sales_intent: "thinking_first",
        approved_reply: "Buleh kita.",
        active: true,
      },
    ],
  };
...
test("ada after the FAQ sales question sends the order form when classifier confirms purchase intent", async () => {
  const customer = {
  ...
  assert.equal(classifyFaqSalesPromptResponse(customer, "ada"), "");
  assert.equal(plan.customerPatch.awaitingPackageBInterest, false);
  assert.equal(plan.customerPatch.pendingOrder.productId, product.id);
  assert.match(plan.messages[0].body, /Full name/);
});

test("ok after the FAQ sales question sends the order form when classifier confirms purchase intent", async () => {
  const customer = {
    id: "customer_ok_accepts_package_b",
    productId: product.id,
    awaitingPackageBInterest: true,
  };
  const plan = await buildConversationPlan({
  ...
  assert.equal(classifyFaqSalesPromptResponse(customer, "ok"), "");
  assert.equal(plan.customerPatch.awaitingPackageBInterest, false);
  assert.equal(plan.customerPatch.pendingOrder.productId, product.id);
  assert.match(plan.messages[0].body, /Full name/);
});
...
test("submitted-order customer can still receive safe general FAQ answers", async () => {
  const plan = await buildConversationPlan({
    catalog,
    faqLibrary,
    customer: {
      id: "customer_submitted_delivery_fee_faq",
      productId: product.id,
      status: "order_submitted",
      followupBlockedReason: "order_submitted",
      orderIds: ["order_123"],
    },
    customerMessage: "delivery ada caj?",
    routeClassification: {
      messageType: "general_faq",
      confidence: "high",
      primaryIntent: "general_delivery_fee",
    },
  });
  ...
  assert.equal(plan.messages[0].body, "nda ya");
  assert.equal(plan.customerPatch.pendingOrder, undefined);
  assert.equal(plan.customerPatch.status, undefined);
  assert.doesNotMatch(plan.messages.map((message) => message.body).join("\n"), /Full name|Phone number|Full address|Package upsell/i);
});
...
test("will update when ready buying words do not open an order form", async () => {
  const plan = await buildConversationPlan({
    catalog,
    customer: {
      id: "customer_will_update_when_ready",
      productId: product.id,
      awaitingPackageBInterest: true,
    },
    customerMessage: "In Shaa Allah will update kalau dh ready nak beli",
    routeClassification: {
      messageType: "purchase_intent",
      confidence: "high",
    },
...
test("nda minat after package-interest question uses approved sales reply", async () => {
  const salesReplyLibrary = {
    sales_replies: [
      {
        id: "sales_not_for_now",
        scope: "business",
        productId: "",
        objection_type: "Not interested",
        intent: "Customer is not interested or wants next time only.",
        example_messages: ["Next time saja", "Nda minat dulu", "Not interested"],
        approved_reply: "Baik kita, no worries. Kalau berminat nanti, boleh message kami semula ya.",
        active: true,
      },
    ],
  ...
  const plan = await buildConversationPlan({
    ...
    salesReplyLibrary,
    customer,
    customerMessage: "nda minat dulu",
    routeClassification: {
      messageType: "sales_reply",
      primaryIntent: "not_interested",
      confidence: "high",
    },
  });

  assert.equal(classifyFaqSalesPromptResponse(customer, "nda minat dulu"), "");
  assert.equal(plan.messages[0].body, "Baik kita, no worries. Kalau berminat nanti, boleh message kami semula ya.");
  assert.equal(plan.messages.length, 1);
  assert.equal(plan.customerPatch.awaitingPackageBInterest, undefined);
  assert.equal(plan.customerPatch.pendingOrder, undefined);
  assert.equal(plan.customerPatch.lastSalesReplyId, "sales_not_for_now");
});
...
test("neutral acknowledgement with thanks does not create human handoff", async () => {
  const plan = await buildConversationPlan({
    catalog,
    customer: {
      id: "customer_says_ok_tq",
      productId: product.id,
    },
    customerMessage: "ok tq",
    routeClassification: {
      messageType: "unknown",
      confidence: "high",
    },
...
test("order wording is not treated as neutral acknowledgement", async () => {
  const plan = await buildConversationPlan({
    catalog,
    customer: {
      id: "customer_says_ok_lock",
      productId: product.id,
    },
    customerMessage: "ok lock promo",
    routeClassification: {
      messageType: "purchase_intent",
      confidence: "high",
    },
  ...
  assert.ok(plan.customerPatch.pendingOrder);
});

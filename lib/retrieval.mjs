...
export function filterKnowledgeRecordsForRoute(records = [], routeClassification = null, product = null) {
  const messageType = String(routeClassification?.messageType || "").trim();
  if (messageType === "general_faq") {
    return (records || []).filter((record) => knowledgeRecordType(record) === "general_faq");
  }
  if (messageType === "product_question") {
    const productId = String(product?.id || "").trim();
    return (records || []).filter((record) => {
      if (knowledgeRecordType(record) === "general_faq") return false;
      const recordProductId = knowledgeRecordProductId(record);
      return !productId || !recordProductId || recordProductId === productId;
    });
  }
  return records || [];
}
...
function knowledgeRecordType(record) {
  return String(
    record?.knowledge_type ||
    record?.scope ||
    record?.kind ||
    record?.metadata?.knowledge_type ||
    record?.metadata?.scope ||
    ""
  ).trim().toLowerCase();
}

function knowledgeRecordProductId(record) {
  return String(
    record?.product_id ||
    record?.productId ||

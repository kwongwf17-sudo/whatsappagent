...
export class WebWhatsAppManager {
  ...
  async start({ onMessage, onRecoveredMessage, accounts = [] } = {}) {
  ...
  async ensureTransport(accountId, { autoStart = false } = {}) {
    ...
    if (!transport) {
      transport = new WebWhatsAppTransport({
        ...
        processFromMeMessages: this.processFromMeMessages,
        logRawInbound: this.logRawInbound,
        rawInboundLogMaxChars: this.rawInboundLogMaxChars,
    ...
    if (autoStart && !transport.started) {
      await transport.start({
        ...
        onRecoveredMessage: async (message) => {
          if (!this.onRecoveredMessage) return;
  ...
  async send(accountId, to, message) {
    const shouldAutoStart = hasSavedSession(this.sessionRootDir, accountId);
    const transport = await this.ensureTransport(accountId, { autoStart: shouldAutoStart });

...
export class OperationsStore {
  ...
  async recordError({ scope, message, accountId = "", details = "" }) {
    if (this.#usesOperationalEventsTable()) {
      const saved = {
        id: `err_${Date.now()}_${Math.floor(Math.random() * 1000000)}`,
      ...
      return this.adapter.insertOperationalEvent("error_log", saved, MAX_LOG_ROWS);
    }
  ...
  async listErrors(accountId = "") {
    if (this.#usesOperationalEventsTable()) {
      return this.adapter.listOperationalEvents("error_log", accountId);
    }
    const db = await this.#readJson(this.errorsPath, { errors: [] });
  ...
  async recordFailedMessage({ businessAccountId = "", to, messages = [], meta = {}, error = "" }) {
    if (this.#usesOperationalEventsTable()) {
      const saved = {
        id: `failed_${Date.now()}_${Math.floor(Math.random() * 1000000)}`,
        createdAt: new Date().toISOString(),
        updatedAt: new Date().toISOString(),
      ...
      return this.adapter.insertOperationalEvent("failed_messages", saved, MAX_LOG_ROWS);
    }
  ...
  async listFailedMessages(businessAccountId = "") {
    if (this.#usesOperationalEventsTable()) {
      return this.adapter.listOperationalEvents("failed_messages", businessAccountId);
    }
    const db = await this.#readJson(this.failedMessagesPath, { messages: [] });
  ...
  async getFailedMessage(id) {
    if (this.#usesOperationalEventsTable()) {
      return this.adapter.getOperationalEvent("failed_messages", id);
    }
    const db = await this.#readJson(this.failedMessagesPath, { messages: [] });
  ...
  async markRetry(id, { success, error = "" }) {
    if (this.#usesOperationalEventsTable()) {
      const message = await this.adapter.getOperationalEvent("failed_messages", id);
      ...
      return this.adapter.insertOperationalEvent("failed_messages", message, MAX_LOG_ROWS);
    }
    const db = await this.#readJson(this.failedMessagesPath, { messages: [] });
  ...
  async resolveNoReply({ businessAccountId = "", customerId, inboundMessageId, actor = "business_admin" }) {
    if (this.#usesOperationalEventsTable()) {
      const saved = {
        id: `review_${Date.now()}_${Math.floor(Math.random() * 1000000)}`,
        createdAt: new Date().toISOString(),
        updatedAt: new Date().toISOString(),
        businessAccountId: String(businessAccountId || ""),
      ...
      return this.adapter.insertOperationalEvent("no_reply_reviews", saved, MAX_LOG_ROWS);
    }
  ...
  async listNoReplyReviews(businessAccountId = "") {
    if (this.#usesOperationalEventsTable()) {
      return this.adapter.listOperationalEvents("no_reply_reviews", businessAccountId);
    }
    const db = await this.#readJson(this.noReplyReviewsPath, { reviews: [] });
  ...
  async enqueueFollowups(items = [], queuedAt = new Date()) {
    ...
    if (this.#usesFollowupQueueTable()) {
      const saved = await this.adapter.enqueueFollowupDispatches(items, queuedAt);
      await this.adapter.pruneFollowupDispatches(this.completedFollowupRetentionMs);
      return saved;
    }
  ...
  async claimFollowupBatch(limit = 1, now = new Date(), businessAccountId = "") {
    ...
    return this.claimFollowupBatches([{ businessAccountId, limit: safeLimit }], now);
  }

  async claimFollowupBatches(accountLimits = [], now = new Date()) {
    const limits = (Array.isArray(accountLimits) ? accountLimits : [])
      .map((entry) => ({
        businessAccountId: String(entry?.businessAccountId || ""),
        limit: Math.max(1, Number(entry?.limit) || 1),
      }));
    if (!limits.length) return [];
    if (this.#usesFollowupQueueTable()) {
      const claimed = await this.adapter.claimFollowupDispatchBatches(limits, now);
      await this.adapter.pruneFollowupDispatches(this.completedFollowupRetentionMs);
      return claimed;
    }
    ...
    return this.#mutateFollowupQueue((db, control) => {
      ...
      const batch = [];
      for (const { businessAccountId, limit } of limits) {
        const accountBatch = db.items
          .filter((item) => {
  ...
  async updateFollowupDispatch(id, patch = {}) {
    if (this.#usesFollowupQueueTable()) {
      return this.adapter.updateFollowupDispatch(id, patch);
    }
    return this.#mutateFollowupQueue((db, control) => {
  ...
  async updateFollowupDispatches(updates = []) {
    ...
    if (this.#usesFollowupQueueTable()) {
      return this.adapter.updateFollowupDispatches(normalized);
    }
    return this.#mutateFollowupQueue((db, control) => {
      const saved = [];
      let changed = false;
  ...
  async listFollowupQueue(businessAccountId = "") {
    if (this.#usesFollowupQueueTable()) {
      return this.adapter.listFollowupDispatches(businessAccountId);
    }
  ...
  async purgeAccountData(businessAccountId = "") {
    ...
    if (this.adapter?.deleteOperationalEventsForAccount) {
      summary.operationalEvents += await this.adapter.deleteOperationalEventsForAccount(accountId);
    }
    if (this.adapter?.deleteFollowupDispatchesForAccount) {
      summary.followupQueue += await this.adapter.deleteFollowupDispatchesForAccount(accountId);
    }

    await this.#purgeJsonOperationalList(this.errorsPath, "errors", accountId, summary, "errors");
    await this.#purgeJsonOperationalList(this.failedMessagesPath, "messages", accountId, summary, "failedMessages");
    await this.#purgeJsonOperationalList(this.noReplyReviewsPath, "reviews", accountId, summary, "noReplyReviews");
    await this.#purgeJsonFollowupQueue(accountId, summary);
    await this.#purgeDashboardProfile(accountId, summary);

    return summary;
  }

  #usesFollowupQueueTable() {
    return Boolean(
      this.adapter?.followupQueueTableEnabled &&
      this.adapter?.enqueueFollowupDispatches &&
      this.adapter?.claimFollowupDispatchBatches &&
      this.adapter?.updateFollowupDispatches &&
      this.adapter?.listFollowupDispatches
    );
  }

  #usesOperationalEventsTable() {
    return Boolean(
      this.adapter?.operationalEventsTableEnabled &&
      this.adapter?.insertOperationalEvent &&
      this.adapter?.listOperationalEvents &&
      this.adapter?.getOperationalEvent
    );
  }
  ...
  async #purgeJsonOperationalList(filePath, property, accountId, summary, summaryKey) {
    const db = await this.#readJsonIfExists(filePath);
    if (!Array.isArray(db?.[property])) return;
    const before = db[property].length;
    db[property] = db[property].filter((item) => !belongsToBusiness(item, accountId));
    const removed = before - db[property].length;
    if (removed) {
      summary[summaryKey] += removed;
      await this.#writeJson(filePath, db);
    }
  }

  async #purgeJsonFollowupQueue(accountId, summary) {
    const db = await this.#readJsonIfExists(this.followupQueuePath);
    if (!Array.isArray(db?.items)) return;
    const before = db.items.length;
    db.items = db.items.filter((item) => !belongsToBusiness(item, accountId));
    const removed = before - db.items.length;
    if (removed) {
      summary.followupQueue += removed;
      await this.#writeJson(this.followupQueuePath, db);
    }
  }

  async #purgeDashboardProfile(accountId, summary) {
    const state = await this.#readJsonIfExists(this.statePath);
    if (!state?.dashboardProfiles || typeof state.dashboardProfiles !== "object" || Array.isArray(state.dashboardProfiles)) {
      return;
    }
    if (!(accountId in state.dashboardProfiles)) return;
    delete state.dashboardProfiles[accountId];
    state.lastUpdatedAt = new Date().toISOString();
    summary.dashboardProfile += 1;
    await this.#writeJson(this.statePath, state);
  }

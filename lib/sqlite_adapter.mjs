...
export class SqliteJsonAdapter {
  ...
  async readJsonIfExists(filePath) {
    await this.ready;
    const row = this.selectDocument.get(documentKey(filePath));
    return row ? JSON.parse(row.value) : null;
  }

  async deleteJsonDocumentKey(key) {
    await this.ready;
    const result = this.deleteDocument.run(String(key || ""));
    return result.changes || 0;
  }

  async deleteJsonDocumentKeysByPrefix(prefix = "") {
    await this.ready;
    const result = this.deleteDocumentsByPrefix.run(`${String(prefix || "")}%`);
    return result.changes || 0;
  }
  ...
  async #initialize() {
    ...
    `);
    this.deleteDocument = this.db.prepare("DELETE FROM json_documents WHERE key = ?");
    this.deleteDocumentsByPrefix = this.db.prepare("DELETE FROM json_documents WHERE key LIKE ?");
  }

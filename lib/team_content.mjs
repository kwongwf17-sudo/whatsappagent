...
export class TeamContentStore {
  ...
  async getContent(accountId, defaults) {
    if (this.#usesTeamContentTables()) {
      return this.#getTableContent(accountId, defaults);
    }
  ...
  async saveContent(accountId, content) {
    if (this.#usesTeamContentTables()) {
      return this.#saveTableContent(accountId, content);
    }
    if (this.adapter?.readJsonIfExists) {
  ...
  async deleteContent(accountId) {
    const key = String(accountId || "default");
    let deleted = 0;
    if (this.#usesTeamContentTables() && this.adapter?.deleteTeamContentRecord) {
      deleted += await this.adapter.deleteTeamContentRecord(key);
    }
    if (this.adapter?.deleteJsonDocumentKey) {
      deleted += await this.adapter.deleteJsonDocumentKey(teamContentDocumentName(key));
    }
    if (this.adapter?.readJsonIfExists) {
      const legacy = await this.adapter.readJsonIfExists(this.path);
      if (legacy?.accounts && Object.prototype.hasOwnProperty.call(legacy.accounts, key)) {
        delete legacy.accounts[key];
        await this.adapter.writeJson(this.path, legacy);
        deleted += 1;
      }
      return { deleted };
    }
    const db = await this.#read();
    if (db.accounts && Object.prototype.hasOwnProperty.call(db.accounts, key)) {
  ...
  async #getTableContent(accountId, defaults) {
    const key = String(accountId || "default");
    const existing = await this.adapter.getTeamContentRecord(key, defaults);
    if (existing) {
      const content = cloneContent(existing);
      const compacted = compactTeamContent(content);
      if (compacted.changed) {
        await this.adapter.saveTeamContentRecord(key, {
          ...content,
          updatedAt: new Date().toISOString(),
          createdAt: existing?.createdAt || new Date().toISOString(),
        });
    ...
    const legacyAccountPath = this.#accountPath(key);
    const legacyAccount = this.adapter?.readJsonIfExists ? await this.adapter.readJsonIfExists(legacyAccountPath) : null;
    const legacyAll = this.adapter?.readJsonIfExists ? await this.adapter.readJsonIfExists(this.path) : null;
    const migrated = legacyAccount || legacyAll?.accounts?.[key] || null;
    if (migrated) {
      const content = cloneContent(migrated);
      compactTeamContent(content);
      return this.adapter.saveTeamContentRecord(key, {
        ...content,
        updatedAt: new Date().toISOString(),
        createdAt: migrated?.createdAt || new Date().toISOString(),
      });
    ...
    seeded.createdAt = new Date().toISOString();
    seeded.updatedAt = seeded.createdAt;
    return this.adapter.saveTeamContentRecord(key, seeded);
  }

  async #saveTableContent(accountId, content) {
    const key = String(accountId || "default");
    const existing = await this.adapter.getTeamContentRecord(key, null);
    const now = new Date().toISOString();
    const compactedContent = cloneContent(content);
    compactTeamContent(compactedContent);
    return this.adapter.saveTeamContentRecord(key, {
  ...
  #usesTeamContentTables() {
    return Boolean(
      this.adapter?.teamContentTableEnabled &&
      this.adapter?.getTeamContentRecord &&
      this.adapter?.saveTeamContentRecord
    );
  }

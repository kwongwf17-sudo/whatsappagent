...
Use the file_search tool as your source of truth for approved embedded knowledge only.
Follow these rules:
...
- The approved embedded knowledge may include general FAQ, product FAQ, approved FAQ, and product image knowledge chunks.
- Do not use sales scripts, follow-up scripts, generic SOPs, or unapproved content as answer sources.
...
export async function createProductImageVectorStoreReply({
  apiKey,
  model,
  vectorStoreId,
  customerMessage,
  productName = "",
  productId = "",
  supportLanguage = "Malay/English",
  maxResults = 8,
}) {
  const systemPrompt = `
You answer WhatsApp customer questions using product image knowledge chunks from file_search.

Use file_search as the source of truth. The vector store contains approved product image chunks.
Active product: ${productName || "unknown product"}${productId ? ` (${productId})` : ""}.

Rules:
- Reply in ${supportLanguage}.
- Answer only from file_search results that belong to the active product.
- Use only approved product image chunk knowledge. Do not use FAQ records, sales scripts, world knowledge, assumptions, or similar products.
- If file_search does not return a direct answer for the active product, set handoff_required true.
- Do not invent stock status, originality/authenticity, side effects, delivery timing, self-collect policy, best-seller status, or usage frequency unless directly present in the retrieved product image chunks.
- For price/package/COD/refund/product feature questions, answer only when those details are directly present in retrieved image chunks.
- Keep the reply short, natural, and customer-facing. Do not mention "file_search", "vector store", "chunk", "image chunk", "retrieved", "source", or "visible text".
- Do not quote raw extracted text as a blob. Rewrite it naturally.
- Do not overclaim beyond the retrieved knowledge. Use soft wording like "boleh bantu" when appropriate.
- Return only valid JSON:
  {"reply":"message to send or empty string","handoff_required":false,"handoff_reason":"short reason when handoff is needed","matched_product":"product name found in retrieved chunks or empty"}
`.trim();
    ...
    body: JSON.stringify({
      ...
      input: [
        { role: "system", content: [{ type: "input_text", text: systemPrompt }] },
        {
          ...
          content: [{
            type: "input_text",
            text: `Active product: ${productName || "unknown"}\nActive product ID: ${productId || "unknown"}\nCustomer message: ${customerMessage}`,
          }],
        },
      ],
      tools: [
  ...
  try {
    ...
    return {
      ...
      matchedProduct: String(parsed.matched_product || "").trim(),
    };
  } catch {
    return {
      reply: "",
...
- Also create Brunei-Malay customer wording for the same knowledge:
  ...
  - brunei_malay_search_text: Brunei/Malay search wording and slang customers may use to ask about this exact image content.
  - brunei_malay_question_examples: example customer questions in Brunei/Malay style about this exact image content.
...
- If a topic is not visible in the image, it must not appear in brunei_malay_summary, brunei_malay_search_text, question_examples, or brunei_malay_question_examples.

...
Use active_state as routing context; it is the current conversation state before this customer message.
First infer whether the customer message is standalone or is answering the latest relevant agent/business question.
For short or ambiguous replies, use recent context to infer the customer's meaning, but do not invent meaning that is not supported by the message and context.
...
- If a short reply answers a previous package, promo, payment, delivery, FAQ, or follow-up question, classify based on the contextual meaning of that answer.
- If active_state is awaitingPackageInterest, interpret the customer message as an answer to the latest package/order-interest question unless it is clearly a new question or objection.
- In awaitingPackageInterest, informal agreement or desire to proceed can be purchase_intent even without explicit words like order/beli/ambil. Examples: "saya mau", "mau tu", "boleh cuba", "jadi", "yang atu".
...
Decide whether this pair should become a reusable inactive template suggestion.

Return type:
- none: not reusable, order details, private customer data, complaint handling, or too specific
- faq: reusable question and approved answer
- sales_reply: reusable objection, hesitation, budget, payday, thinking-first, not-interested, another-date, or negotiation reply

For sales_reply, use only one listed SALES_INTENT id. If none fits, use an empty sales_intent.
Do not invent product facts. Do not activate anything.

Return only valid JSON:
{"type":"none|faq|sales_reply","scope":"general|product","topic":"short topic","sales_intent":"","reason":"short reason"}
`.trim();
  ...
  const type = ["faq", "sales_reply"].includes(parsed.type) ? parsed.type : "none";
  const allowedSalesIntents = new Set(salesIntents.map((item) => item.id).filter(Boolean));
  const salesIntent = allowedSalesIntents.has(parsed.sales_intent) ? parsed.sales_intent : "";
  return {
    type,
    scope: parsed.scope === "product" ? "product" : "general",

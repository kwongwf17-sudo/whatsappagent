...
export async function deleteVectorStore(apiKey, vectorStoreId) {
  return openaiFetch(apiKey, `/vector_stores/${vectorStoreId}`, {
    method: "DELETE",
  });
}
...
Choose a SALES_REPLY_ID only when the customer is clearly showing a buying objection, hesitation, concern, or sales-related response.
Examples of sales intent include: asking first, thinking first, waiting for payday, too expensive, scared it is not suitable, asking spouse/family first, comparing whether to buy, or needing reassurance before purchase.

import { readFile } from "node:fs/promises";
import path from "node:path";
import { sanitizeImageKnowledgeChunk } from "./knowledge_sanitizer.mjs";

const OPENAI_BASE_URL = "https://api.openai.com/v1";

async function openaiFetch(apiKey, endpoint, options = {}) {
  const headers = {
    Authorization: `Bearer ${apiKey}`,
    ...(options.body instanceof FormData ? {} : { "Content-Type": "application/json" }),
    ...(options.headers || {}),
  };

  const response = await fetch(`${OPENAI_BASE_URL}${endpoint}`, {
    ...options,
    headers,
  });

  const text = await response.text();
  let data = {};
  if (text) {
    try {
      data = JSON.parse(text);
    } catch {
      const preview = text.replace(/\s+/g, " ").slice(0, 200);
      throw new Error(`OpenAI ${endpoint} returned non-JSON response: ${preview}`);
    }
  }
  if (!response.ok) {
    const message = data?.error?.message || response.statusText;
    throw new Error(`OpenAI ${endpoint} failed: ${message}`);
  }
  return data;
}

export async function createVectorStore(apiKey, name) {
  return openaiFetch(apiKey, "/vector_stores", {
    method: "POST",
    body: JSON.stringify({ name }),
  });
}

export async function uploadFile(apiKey, filePath) {
  const body = new FormData();
  const bytes = await readFile(filePath);
  body.append("purpose", "assistants");
  body.append("file", new Blob([bytes]), path.basename(filePath));

  return openaiFetch(apiKey, "/files", {
    method: "POST",
    body,
  });
}

export async function attachFileToVectorStore(apiKey, vectorStoreId, fileId) {
  return openaiFetch(apiKey, `/vector_stores/${vectorStoreId}/files`, {
    method: "POST",
    body: JSON.stringify({ file_id: fileId }),
  });
}

export async function listVectorStoreFiles(apiKey, vectorStoreId) {
  const files = [];
  let after = "";
  do {
    const query = new URLSearchParams({ limit: "100" });
    if (after) query.set("after", after);
    const data = await openaiFetch(apiKey, `/vector_stores/${vectorStoreId}/files?${query.toString()}`);
    const batch = Array.isArray(data.data) ? data.data : [];
    files.push(...batch);
    after = data.has_more && batch.length ? batch.at(-1).id : "";
  } while (after);
  return files;
}

export async function deleteVectorStoreFile(apiKey, vectorStoreId, fileId) {
  return openaiFetch(apiKey, `/vector_stores/${vectorStoreId}/files/${fileId}`, {
    method: "DELETE",
  });
}

export async function deleteUploadedFile(apiKey, fileId) {
  return openaiFetch(apiKey, `/files/${fileId}`, {
    method: "DELETE",
  });
}

export async function transcribeAudio({ apiKey, model = "gpt-4o-mini-transcribe", bytes, filename = "voice.ogg", mimeType = "audio/ogg" }) {
  if (!bytes?.length) return "";
  const body = new FormData();
  body.append("model", model);
  body.append("file", new Blob([bytes], { type: mimeType || "application/octet-stream" }), filename || "voice.ogg");

  const data = await openaiFetch(apiKey, "/audio/transcriptions", {
    method: "POST",
    body,
  });
  return String(data.text || "").trim();
}

export async function interpretCustomerImage({
  apiKey,
  model,
  imageDataUrl,
  caption = "",
  productName = "",
  conversationContext = [],
}) {
  const systemPrompt = `
You inspect a customer-sent WhatsApp image for a sales/support chatbot.

Return only valid JSON:
{"category":"customer_question|order_details|payment_proof|complaint_or_damage|unclear","safe_to_auto_route":false,"text":"short text to feed into the existing chatbot router","reason":"short reason"}

Rules:
- Do not answer the customer.
- Extract only what is visible in the image or written in the caption.
- If the image is a bank slip, payment proof, transfer receipt, damaged/wrong item, medical/safety complaint, angry complaint, private document, or unclear content, set safe_to_auto_route false.
- Use payment_proof for bank/transfer/payment receipts, even if customer asks whether payment is received.
- Use complaint_or_damage for damaged item, wrong item, refund/return evidence, or negative proof photos.
- Use order_details only when the image clearly contains customer order fields such as name, phone, address, package/option/add-on.
- Use customer_question when the image/caption clearly asks a normal product, FAQ, price, package, delivery, stock, or sales question and is not risky.
- The text field should be a concise customer-message equivalent, in the customer's language where possible.
- If caption is present, combine caption with relevant image text.
- If not safe, keep text short and neutral, enough for admin context only.
`.trim();

  const contextText = (conversationContext || [])
    .slice(-6)
    .map((item) => `${item.role || "unknown"}: ${item.text || ""}`)
    .filter((line) => line.trim())
    .join("\n");

  const data = await openaiFetch(apiKey, "/responses", {
    method: "POST",
    body: JSON.stringify({
      model,
      input: [
        { role: "system", content: [{ type: "input_text", text: systemPrompt }] },
        {
          role: "user",
          content: [
            {
              type: "input_text",
              text: [
                `Active product: ${productName || "unknown"}`,
                `Caption: ${caption || "(none)"}`,
                "",
                "Recent conversation:",
                contextText || "(none)",
              ].join("\n"),
            },
            {
              type: "input_image",
              image_url: imageDataUrl,
            },
          ],
        },
      ],
    }),
  });

  try {
    const parsed = JSON.parse(extractOutputText(data));
    const category = String(parsed.category || "").trim();
    return {
      category: ["customer_question", "order_details", "payment_proof", "complaint_or_damage", "unclear"].includes(category) ? category : "unclear",
      safeToAutoRoute: Boolean(parsed.safe_to_auto_route),
      text: String(parsed.text || "").trim(),
      reason: String(parsed.reason || "").trim(),
    };
  } catch {
    return {
      category: "unclear",
      safeToAutoRoute: false,
      text: "",
      reason: "Model returned non-JSON output.",
    };
  }
}

export async function embedTexts({ apiKey, model = "text-embedding-3-small", texts = [] }) {
  const input = (texts || []).map((text) => String(text || ""));
  if (!input.length) return [];
  const data = await openaiFetch(apiKey, "/embeddings", {
    method: "POST",
    body: JSON.stringify({ model, input }),
  });
  return (data.data || [])
    .sort((a, b) => Number(a.index || 0) - Number(b.index || 0))
    .map((item) => item.embedding);
}

export async function searchVectorStore({ apiKey, vectorStoreId, query, maxResults = 3 }) {
  const data = await openaiFetch(apiKey, `/vector_stores/${vectorStoreId}/search`, {
    method: "POST",
    body: JSON.stringify({
      query: String(query || ""),
      max_num_results: maxResults,
    }),
  });
  return (Array.isArray(data.data) ? data.data : []).map((item, index) => {
    const contentText = (Array.isArray(item.content) ? item.content : [])
      .map((part) => part?.text || part?.content || "")
      .filter(Boolean)
      .join("\n");
    return {
      id: item.id || item.file_id || `vector_result_${index + 1}`,
      vector_store_result_id: item.id || "",
      file_id: item.file_id || "",
      filename: item.filename || "",
      score: item.score,
      attributes: item.attributes || {},
      knowledge_type: "vector_store_chunk",
      text: contentText || item.text || "",
    };
  }).filter((item) => item.text);
}

export async function getVectorStoreFile(apiKey, vectorStoreId, fileId) {
  return openaiFetch(apiKey, `/vector_stores/${vectorStoreId}/files/${fileId}`);
}

export async function createCustomerServiceResponse({
  apiKey,
  model,
  vectorStoreId,
  businessName,
  supportLanguage,
  customerId,
  customerMessage,
  normalizedCustomerMessage = "",
  retrievalQuery = "",
  rerankedKnowledgeContext = "",
  productName = "",
  productId = "",
  maxResults = 6,
  useFileSearch = false,
}) {
  const sourceInstruction = useFileSearch
    ? "Use file_search and the provided reranked approved knowledge context as the source of truth."
    : "Use only the provided reranked approved vector-store context as the source of truth.";
  const systemPrompt = `
You are a WhatsApp customer service agent for ${businessName}, serving Brunei customers.
You are fluent in English and Brunei-Malay.

Reply in the same language as the customer. Use natural Brunei-Malay when the customer writes Malay.
Reply like a human: understand the customer's emotion and intent first, then answer warmly and naturally.
Keep WhatsApp replies concise, clear, and easy to scan.

Use only approved knowledge provided by the system:
- local approved FAQ replies that the application has already checked before this call
- local approved sales replies that the application has already checked before this call
- OpenAI vector-store knowledge containing approved general FAQ, approved product FAQ, and approved extracted product image knowledge

Strict source rules:
- ${sourceInstruction}
- Use only information found in that approved knowledge context or explicitly present in the provided customer message.
- The reranked approved knowledge context contains vector-store search results after reranking. Use it only when it directly answers the customer's message.
- For this customer-service answer call, do not answer sales objections from vector-store sales reply records. Approved sales replies are selected by a separate sales-reply selector before this call.
- Do not invent answers.
- Do not use world knowledge.
- Do not answer from SOP, reply flows, sales scripts, generic SOPs, or unapproved notes.
- Do not answer from assumptions, likely/common product behavior, similar products, marketing guesses, or partial context.
- Active product context: ${productName || "unknown product"}${productId ? ` (${productId})` : ""}.
- Do not answer product-specific details for any product other than the active product.
- If the customer asks product-specific details and the approved knowledge context is for another product, set handoff_required to true.
- For product answers, rephrase the approved knowledge into a short human WhatsApp reply.
- Do not mention vector store, file_search, file search, image chunk, retrieved chunk, extracted text, visible text, source, or internal system wording.
- Use the retrieval query only to understand retrieval intent. Preserve the customer's original meaning when writing the final reply.
- Set reply_type to "faq" when answering a product or general business information question from the knowledge base.
- Set reply_type to "other" for non-FAQ replies, clarification questions, complaints, or human-review responses.
- Do not add a sales question after an FAQ answer; the application sends the approved sales follow-up separately.
- Do not invent prices, stock, shipping timelines, refund eligibility, ingredients, warranty terms, or policy details.
- If the knowledge base does not directly answer the question, do not guess. Set handoff_required to true, reply_type to "other", and write a short, natural customer-facing handoff reply in the customer's language.
- For handoff_required replies, do not mention the business name or active product unless the customer's raw message explicitly names that business/product or directly asks about it.
- For unrelated messages, spam links, or unclear messages, do not turn the reply into product support wording. Keep the handoff reply neutral and brief.
- Never request full card numbers, passwords, OTPs, or unnecessary personal data.
- For refunds, complaints, damaged goods, angry customers, legal threats, account-specific order checks, or anything outside SOP, set handoff_required to true.
- Return only valid JSON with this shape:
  {"reply":"message to send to customer","reply_type":"faq|other","handoff_required":false,"handoff_reason":""}
`.trim();

  const payload = {
    model,
    input: [
      {
        role: "system",
        content: [{ type: "input_text", text: systemPrompt }],
      },
      {
        role: "user",
        content: [
          {
            type: "input_text",
            text: `Customer WhatsApp ID: ${customerId}\nActive product: ${productName || "unknown"}\nActive product ID: ${productId || "unknown"}\nRaw customer message: ${customerMessage}\nNormalized customer message: ${normalizedCustomerMessage || customerMessage}\nRetrieval query: ${retrievalQuery || normalizedCustomerMessage || customerMessage}\n\nReranked approved knowledge context:\n${rerankedKnowledgeContext || "(none)"}`,
          },
        ],
      },
    ],
    ...(useFileSearch ? {
      tools: [
        {
          type: "file_search",
          vector_store_ids: [vectorStoreId],
          max_num_results: maxResults,
        },
      ],
    } : {}),
  };

  const data = await openaiFetch(apiKey, "/responses", {
    method: "POST",
    body: JSON.stringify(payload),
  });

  const text = extractOutputText(data);
  return parseAgentJson(text);
}

export async function classifyCustomerMessageRoute({
  apiKey,
  model,
  customerMessage,
  normalizedCustomerMessage = "",
  productName = "",
  activeState = "",
  conversationContext = [],
  faqTopics = [],
  salesIntents = [],
}) {
  const messageTypes = [
    "general_faq",
    "product_question",
    "sales_reply",
    "order_status",
    "complaint",
    "purchase_intent",
    "human_request",
    "unknown",
  ];
  const faqTopicText = faqTopics.length
    ? faqTopics.map((item) => {
        const examples = (item.exampleQuestions || item.example_questions || [])
          .map((question) => String(question || "").trim())
          .filter(Boolean)
          .slice(0, 8);
        return [
          `- ${item.id}: ${item.label || item.topic || item.id}`,
          examples.length ? `  Example customer questions: ${examples.join(" | ")}` : "",
        ].filter(Boolean).join("\n");
      }).join("\n")
    : "- none";
  const salesIntentText = salesIntents.length
    ? salesIntents.map((item) => `- ${item.id}: ${item.label || item.id}`).join("\n")
    : "- none";
  const recentText = (conversationContext || [])
    .slice(-8)
    .map((message) => `${message.direction || "message"}: ${message.body || message.text || ""}`)
    .join("\n");

  const systemPrompt = `
You classify a WhatsApp customer message for a Brunei customer-service sales agent.

Never classify the latest customer message in isolation.
Always infer the customer's meaning from the latest customer message, the immediate previous bot/team message, recent conversation history, active_state, and locked product/order context.
Use active_state as helpful routing context; it is the current conversation state before this customer message, but it is not automatic intent.
First infer whether the customer message is standalone or is answering the immediate previous bot/team message.
If the customer message replies to the immediate previous bot/team message, classify it based on what that previous message asked or implied.
For short, ambiguous, acknowledgement-style, or context-dependent replies, the immediate previous message is especially important, but do not invent meaning that is not supported by the message and context.
Return one message_type from this exact list: ${messageTypes.join(", ")}.
Return primary_intent only from the listed FAQ_TOPIC ids or SALES_INTENT ids. Do not invent intent names.
If no listed intent fits, primary_intent must be empty.
Use FAQ_TOPIC labels and example customer questions as meaning clues when choosing the closest supported FAQ_TOPIC id.

Routing rules:
- product_question: asks about active product facts, benefits, usage, suitability, price, package, stock, reviews, side effects, ingredients, warranty, or product-specific delivery.
- general_faq: asks business-level questions like delivery area, COD, pickup, location, payment, or general stock timing.
- sales_reply: customer hesitation, objection, not interested, payday, expensive, thinking first, or another date purchase.
- order_status: asks about an existing submitted order or delivery progress.
- complaint: damaged/wrong/missing/late/refund/angry/poor service.
- purchase_intent: wants to buy/order/lock/proceed or is submitting order details, only when there is no stronger objection, hesitation, not-interested, payday, or another-date meaning.
- human_request: directly asks for staff/admin/human.
- unknown: unclear or unsupported.

Priority rules:
- Objection/hesitation beats purchase intent when both appear in one message.
- If the customer says price is expensive, cannot afford, no budget, sorry/nevermind, or will buy later, classify sales_reply even if words like beli/buy/order appear.
- If the customer says they will update/message later when ready, classify sales_reply, not purchase_intent. Examples: "will update kalau dh ready nak beli", "nanti kalau ready saya beli", "kalau jadi nanti ku update", "belum ready nak beli".
- Only classify purchase_intent for clear proceed/order messages such as package choice, lock promo, book/order now, or submitted order details.
- If a reply answers a previous package, promo, payment, delivery, FAQ, or follow-up question, classify based on the contextual meaning of that answer.
- If active_state is awaitingPackageInterest, use it as context, but do not let it override the immediate previous bot/team message.
- Do not classify as purchase_intent only because active_state is awaitingPackageInterest; the latest message must clearly mean proceed/order/choose package after considering the immediate previous message and recent context.
- In awaitingPackageInterest, informal agreement or desire to proceed can be purchase_intent when the immediate previous message or recent context clearly asked about ordering, locking promo, package interest, or package choice. Examples: "saya mau", "mau tu", "boleh cuba", "jadi", "yang atu".
- In awaitingPackageInterest, clear new questions still keep their route: delivery/payment/location questions are general_faq; product/price/package questions are product_question; hesitation/budget/later/not-interested answers are sales_reply.
- Distinguish payment questions from sales hesitation: asking whether payday/payment is allowed is general_faq when a matching FAQ_TOPIC exists; saying to wait for payday or not enough money is sales_reply.
- Do not force a sales_reply for factual FAQ, product questions, order details, order status, complaints, opt-outs, or unrelated chat.

Set clarification_required true when the message is safe but too ambiguous to route confidently and the bot should ask one clarification question.
Set human_support_required true for complaint, human_request, unsafe, order-risk, refund/damage, unsupported, or repeated unclear messages needing admin.

Return only valid JSON:
{"message_type":"general_faq|product_question|sales_reply|order_status|complaint|purchase_intent|human_request|unknown","primary_intent":"","related_product":"","clarification_required":false,"human_support_required":false,"confidence":"high|medium|low","reason":"short reason"}
`.trim();

  const data = await openaiFetch(apiKey, "/responses", {
    method: "POST",
    body: JSON.stringify({
      model,
      input: [
        { role: "system", content: [{ type: "input_text", text: systemPrompt }] },
        {
          role: "user",
          content: [{
            type: "input_text",
            text: [
              `Active product: ${productName || "unknown"}`,
              `Active state: ${activeState || "none"}`,
              `Raw customer message: ${customerMessage}`,
              `Normalized customer message: ${normalizedCustomerMessage || customerMessage}`,
              "",
              "Recent conversation:",
              recentText || "(none)",
              "",
              "FAQ_TOPIC ids:",
              faqTopicText,
              "",
              "SALES_INTENT ids:",
              salesIntentText,
            ].join("\n"),
          }],
        },
      ],
    }),
  });

  return normalizeRouteClassification(extractOutputText(data), messageTypes, faqTopics, salesIntents);
}

export async function suggestTemplateImprovement({
  apiKey,
  model,
  customerMessage,
  adminReply,
  productName = "",
  productId = "",
  existingGeneralFaqs = [],
  existingProductFaqs = [],
}) {
  const formatFaqs = (records = []) => records.length
    ? records.map((item) => [
      `- ID: ${item.id}`,
      `  Topic: ${item.topic || item.label || item.id}`,
      `  Examples: ${(item.exampleQuestions || item.example_questions || []).slice(0, 5).join(" | ") || "none"}`,
    ].join("\n")).join("\n")
    : "- none";
  const systemPrompt = `
You review one WhatsApp customer message and the admin's manual reply.
Decide whether this pair should become a reusable inactive FAQ suggestion.

Return type:
- none: not reusable, order details, private customer data, complaint handling, or too specific
- general_faq: reusable business-level FAQ and approved answer
- product_faq: reusable product-specific FAQ and approved answer

Do not create sales replies. Sales objections, hesitation, budget/payday, thinking-first, not-interested, another-date, negotiation, or closing replies must be "none".
Do not create suggestions from order details, names, phone numbers, addresses, package choices, delivery arrangements, complaint handling, media/voice/image handoff replies, or one-off admin actions.
Only suggest FAQ when the customer asked a reusable factual question and the admin reply is a reusable factual answer.

Use existing FAQ topics when the meaning is already covered:
- Set existing_topic_id to the matching ID.
- Keep topic as the existing topic wording.
- Do not create a duplicate topic with similar meaning.

Create a new topic only when no existing topic covers the meaning.
New topic wording must be short, clean, reusable, and professional. Do not copy customer slang unless it is the real business/product term.
The topic should describe the question category, not the reply.
Do not invent product facts. Do not activate anything.

Return only valid JSON:
{"type":"none|general_faq|product_faq","existing_topic_id":"","topic":"short topic","reason":"short reason"}
`.trim();

  const data = await openaiFetch(apiKey, "/responses", {
    method: "POST",
    body: JSON.stringify({
      model,
      input: [
        { role: "system", content: [{ type: "input_text", text: systemPrompt }] },
        {
          role: "user",
          content: [{
            type: "input_text",
            text: [
              `Active product: ${productName || "unknown"}`,
              `Active product ID: ${productId || "unknown"}`,
              `Customer message: ${customerMessage}`,
              `Admin manual reply: ${adminReply}`,
              "",
              "Existing General FAQ topics:",
              formatFaqs(existingGeneralFaqs),
              "",
              "Existing Product FAQ topics for the active product:",
              formatFaqs(existingProductFaqs),
            ].join("\n"),
          }],
        },
      ],
    }),
  });

  let parsed = {};
  try {
    parsed = JSON.parse(extractOutputText(data));
  } catch {
    parsed = {};
  }
  const rawType = String(parsed.type || "").trim();
  const type = rawType === "general_faq" || rawType === "product_faq" ? "faq" : "none";
  const scope = rawType === "product_faq" ? "product" : "general";
  const validExistingIds = new Set([
    ...existingGeneralFaqs.map((item) => String(item.id || "")).filter(Boolean),
    ...existingProductFaqs.map((item) => String(item.id || "")).filter(Boolean),
  ]);
  const existingTopicId = validExistingIds.has(parsed.existing_topic_id) ? parsed.existing_topic_id : "";
  return {
    type,
    scope,
    topic: String(parsed.topic || "").trim().slice(0, 100),
    existingTopicId,
    reason: String(parsed.reason || "").trim().slice(0, 200),
  };
}

export async function rerankKnowledgeRecords({
  apiKey,
  model,
  customerMessage,
  normalizedCustomerMessage = "",
  route = null,
  records = [],
  topK = 3,
}) {
  const candidates = (records || []).filter(Boolean).slice(0, 12);
  if (!candidates.length) return [];
  const validIds = new Set(candidates.map((record) => String(record.id || "")));
  const candidateText = candidates.map((record, index) => [
    `#${index + 1}`,
    `ID: ${record.id}`,
    `TYPE: ${record.knowledge_type || record.scope || record.kind || ""}`,
    `TOPIC_OR_CATEGORY: ${record.topic || record.category || record.title || ""}`,
    `TEXT: ${knowledgeRecordText(record).slice(0, 1200)}`,
  ].join("\n")).join("\n\n");

  const systemPrompt = `
You rerank approved knowledge candidates for a WhatsApp customer question.

Use only the listed candidate IDs. Do not invent IDs.
Rank candidates by direct answer quality for the customer's actual question.
Prefer the predicted primary intent/message type only as a soft boost; ignore it if another candidate clearly answers better.
Return up to ${topK} IDs.

Return only valid JSON:
{"top_ids":["id1","id2","id3"],"reason":"short reason"}
`.trim();

  const data = await openaiFetch(apiKey, "/responses", {
    method: "POST",
    body: JSON.stringify({
      model,
      input: [
        { role: "system", content: [{ type: "input_text", text: systemPrompt }] },
        {
          role: "user",
          content: [{
            type: "input_text",
            text: [
              `Raw customer message: ${customerMessage}`,
              `Normalized customer message: ${normalizedCustomerMessage || customerMessage}`,
              `Predicted message type: ${route?.messageType || ""}`,
              `Predicted primary intent: ${route?.primaryIntent || ""}`,
              "",
              "Candidates:",
              candidateText,
            ].join("\n"),
          }],
        },
      ],
    }),
  });

  try {
    const parsed = JSON.parse(extractOutputText(data));
    const ids = (Array.isArray(parsed.top_ids) ? parsed.top_ids : [])
      .map((id) => String(id || "").trim())
      .filter((id) => validIds.has(id))
      .slice(0, topK);
    return ids.map((id) => candidates.find((record) => String(record.id || "") === id)).filter(Boolean);
  } catch {
    return candidates.slice(0, topK);
  }
}

export async function extractProductKnowledgeFromImage({
  apiKey,
  model,
  productName,
  imageDataUrl,
  imageLabel = "product image",
}) {
  const systemPrompt = `
You extract one image-level knowledge chunk from a product image for a WhatsApp sales agent.

Rules:
- Extract only facts visible in the image or clearly written in the image.
- Do not guess missing facts.
- Do not use product catalog knowledge, other images, brand knowledge, common sense, or likely customer questions to fill missing details.
- Preserve the meaning of the image, but avoid turning marketing text into stronger claims.
- Do not create medical, cure, guaranteed-result, or permanent-result claims.
- Create exactly one image_chunk that summarizes the whole image as a searchable knowledge source.
- The image_chunk must stay grounded to this image only and should mention visible headings, sections, and important text.
- If the image/file label includes words like benefit, feature, price, ingredient, usage, warning, testimonial, use that clue for image_chunk.category only when it matches the visible image content.
- Use a precise category for image_chunk.category:
  specification, ingredient, usage, price, package_option, add_on, feature, benefit_claim, problem_shown, caution, delivery, warranty_refund, stock_timeline, social_proof, other
- Use "benefit_claim" only for marketing benefits written or clearly shown in the image.
- Use "problem_shown" for before-side or problem labels like blackheads, oily T-zone, clogged pores.
- Use "feature" for product capabilities like suction modes, heads, rechargeable, button.
- Use "specification" for measurable facts like volume, size, quantity, material, battery, power.
- Do not create separate fact rows. Put all useful visible content inside image_chunk.summary, image_chunk.extracted_text, and image_chunk.embedding_text.

English fields:
- summary: customer-safe English summary of only visible image content.
- extracted_text: important visible text copied from the image.
- embedding_text: searchable English text using only the product name, image label, category, summary, and visible text.

Brunei-Malay fields:
- brunei_malay_summary: strict Brunei-Malay translation or very light paraphrase of summary + extracted_text only.
- brunei_malay_search_text: Brunei-Malay equivalents of words/facts already present in summary, extracted_text, or embedding_text only.
- Use natural Brunei-Malay wording where possible. Keep English only for brand names, prices, model names, units, exact visible product terms, or words Brunei customers normally use as-is.

Strict grounding rules:
- Do not create broad customer-search keywords, likely FAQ topics, sales keywords, or question examples for image chunks.
- Do not add price, promo, package, discount, COD, delivery, refund, warranty, side-effect, sensitive-skin, review, stock, or origin wording unless that topic is visibly present in the image.
- If a topic is not visible in the image, it must not appear in brunei_malay_summary or brunei_malay_search_text.
- Prefer customer-answerable content over vague words, but keep visible marketing/problem text inside the chunk when it helps retrieval.
- Set customer_safe false when the image content is too risky to answer customers without human review.
- Return only valid JSON:
{"image_chunk":{"category":"feature","title":"Product features","summary":"Short customer-safe English summary","extracted_text":"Important visible text from the image","embedding_text":"Searchable English text combining product, image label, category, summary, and visible text only","brunei_malay_summary":"Strict Brunei-Malay translation/paraphrase of visible image content only","brunei_malay_search_text":"Brunei-Malay equivalents of visible words/facts only","customer_safe":true,"approval_note":"Visible in image"},"facts":[]}
`.trim();

  const data = await openaiFetch(apiKey, "/responses", {
    method: "POST",
    body: JSON.stringify({
      model,
      input: [
        { role: "system", content: [{ type: "input_text", text: systemPrompt }] },
        {
          role: "user",
          content: [
            {
              type: "input_text",
              text: `Product: ${productName}\nImage label: ${imageLabel}\nExtract one image_chunk from this image. Do not create separate fact rows.`,
            },
            {
              type: "input_image",
              image_url: imageDataUrl,
            },
          ],
        },
      ],
    }),
  });

  try {
    const parsed = JSON.parse(extractOutputText(data));
    return {
      imageChunk: normalizeImageChunk(parsed.image_chunk),
      facts: [],
    };
  } catch {
    return { imageChunk: null, facts: [] };
  }
}

function normalizeImageChunk(chunk) {
  if (!chunk || typeof chunk !== "object") return null;
  const title = String(chunk.title || "").trim();
  const summary = String(chunk.summary || "").trim();
  const extractedText = String(chunk.extracted_text || "").trim();
  const embeddingText = String(chunk.embedding_text || "").trim();
  const bruneiMalaySummary = String(chunk.brunei_malay_summary || "").trim();
  const bruneiMalaySearchText = String(chunk.brunei_malay_search_text || "").trim();
  if (!title && !summary && !extractedText && !embeddingText) return null;
  return sanitizeImageKnowledgeChunk({
    category: normalizeExtractionCategory(chunk.category),
    title: title || "Image knowledge",
    summary,
    extracted_text: extractedText,
    embedding_text: embeddingText || [title, summary, extractedText].filter(Boolean).join("\n"),
    brunei_malay_summary: bruneiMalaySummary,
    brunei_malay_search_text: bruneiMalaySearchText,
    customer_safe: chunk.customer_safe !== false,
    approval_note: String(chunk.approval_note || "").trim(),
  });
}

function normalizeExtractionCategory(value) {
  const category = String(value || "other").trim().toLowerCase();
  return [
    "specification",
    "ingredient",
    "usage",
    "price",
    "package_option",
    "add_on",
    "feature",
    "benefit_claim",
    "problem_shown",
    "caution",
    "delivery",
    "warranty_refund",
    "stock_timeline",
    "social_proof",
    "other",
  ].includes(category) ? category : "other";
}

export async function selectApprovedFaq({
  apiKey,
  model,
  customerMessage,
  productName,
  faqRecords = [],
}) {
  if (!faqRecords.length) return null;
  const candidateText = faqRecords
    .map((faq) => [
      `FAQ_ID: ${faq.id}`,
      `SCOPE: ${faq.scope}${faq.product_id ? ` (${productName})` : ""}`,
      `TOPIC: ${faq.topic}`,
      "EXAMPLE CUSTOMER QUESTIONS:",
      ...(faq.example_questions || []).map((question) => `- ${question}`),
    ].join("\n"))
    .join("\n\n");

  const systemPrompt = `
You match a WhatsApp customer question to an approved FAQ record.

Choose an FAQ_ID only when the customer's meaning is clearly the same as a listed topic or example.
The approved reply will be sent by the application, so do not write a customer reply.
Do not match messages that are orders, order-detail submissions, opt-outs, complaints, refund/damage issues, or unrelated chat.
Use the product-specific records only for ${productName}; general records apply to any product.
Do not use outside knowledge or infer an answer from a similar FAQ.
If the FAQ does not directly answer the exact customer question, return no_match.
If uncertain, return no_match.

Return only valid JSON:
{"faq_id":"matching id or empty","match":"high|no_match","reason":"short reason"}
`.trim();
  const payload = {
    model,
    input: [
      { role: "system", content: [{ type: "input_text", text: systemPrompt }] },
      {
        role: "user",
        content: [{
          type: "input_text",
          text: `Customer message: ${customerMessage}\n\nApproved FAQ records:\n${candidateText}`,
        }],
      },
    ],
  };
  const data = await openaiFetch(apiKey, "/responses", {
    method: "POST",
    body: JSON.stringify(payload),
  });
  const text = extractOutputText(data);
  try {
    const parsed = JSON.parse(text);
    const faqId = String(parsed.faq_id || "").trim();
    const match = String(parsed.match || "").trim().toLowerCase();
    if (match !== "high" || !faqId) return null;
    return {
      faqId,
      reason: String(parsed.reason || "").trim(),
    };
  } catch {
    return null;
  }
}

export async function selectSalesReply({
  apiKey,
  model,
  customerMessage,
  normalizedCustomerMessage = "",
  productName,
  salesReplyRecords = [],
  conversationContext = [],
  activeState = "",
  routePrimaryIntent = "",
  routeReason = "",
}) {
  if (!salesReplyRecords.length) return null;
  const recentText = (conversationContext || [])
    .slice(-8)
    .map((message) => `${message.direction || "message"}: ${message.body || message.text || ""}`)
    .join("\n");
  const candidateText = salesReplyRecords
    .map((reply) => [
      `SALES_REPLY_ID: ${reply.id}`,
      `SCOPE: ${reply.scope}${reply.product_id ? ` (${productName})` : ""}`,
      `SALES_INTENT: ${reply.sales_intent || reply.intent_key || ""}`,
      `REPEAT_ACTION: ${reply.repeat_action || "openai_acknowledge"}`,
      `OBJECTION TYPE: ${reply.objection_type || reply.topic || ""}`,
      `INTENT: ${reply.intent || ""}`,
      "EXAMPLE CUSTOMER MESSAGES:",
      ...(reply.example_messages || []).map((message) => `- ${message}`),
    ].join("\n"))
    .join("\n\n");

  const systemPrompt = `
You classify a WhatsApp customer sales response against approved Sales Intent records.

Analyze the customer's meaning from the raw message, normalized message, recent context, and approved sales records.
First infer whether the customer message is standalone or is answering the latest relevant agent/business question.
Use active state and route classifier intent as context hints only; choose the approved sales reply that best fits the full conversation meaning.
For short or ambiguous replies, use recent context to infer the customer's sales meaning, but do not invent meaning that is not supported by the message and context.
Treat common typo, spelling, Brunei-Malay, and shorthand variants as the same meaning, for example tunggu/tungu, belum/blom/balum, budget/bajet, gaji/payday, pakej/package.
Choose the closest clearly-supported approved SALES_REPLY_ID based on customer meaning, recent context, SALES_INTENT, objection type, intent description, and examples.
Examples guide the meaning but do not limit matching; do not require exact wording when the intent is clear.
The approved reply will be sent by the application, so do not write a customer reply.
Do not match factual product questions, order-detail submissions, order status enquiries, opt-outs, complaints, refund/damage issues, legal/report threats, or unrelated chat.
Use the product-specific records only for ${productName}; general records apply to any product.
Do not use file_search, vector stores, outside knowledge, or invented intent names.
Do not choose a broad hesitation when a more specific approved sales intent clearly fits.
If the customer meaning is not clearly a sales response or no approved intent is clearly supported, return no_match.
If uncertain, return no_match.

Return only valid JSON:
{"sales_reply_id":"matching id or empty","match":"high|no_match","reason":"short reason"}
`.trim();
  const data = await openaiFetch(apiKey, "/responses", {
    method: "POST",
    body: JSON.stringify({
      model,
      input: [
        { role: "system", content: [{ type: "input_text", text: systemPrompt }] },
        {
          role: "user",
          content: [{
            type: "input_text",
            text: [
              `Raw customer message: ${customerMessage}`,
              `Normalized customer message: ${normalizedCustomerMessage || customerMessage}`,
              `Active state: ${activeState || "none"}`,
              `Route classifier sales-intent hint: ${routePrimaryIntent || "none"}`,
              `Route classifier reason: ${routeReason || "none"}`,
              "",
              "Recent conversation:",
              recentText || "(none)",
              "",
              "Approved sales reply records:",
              candidateText,
            ].join("\n"),
          }],
        },
      ],
    }),
  });
  const text = extractOutputText(data);
  try {
    const parsed = JSON.parse(text);
    const salesReplyId = String(parsed.sales_reply_id || "").trim();
    const match = String(parsed.match || "").trim().toLowerCase();
    if (match !== "high" || !salesReplyId) return null;
    return {
      salesReplyId,
      reason: String(parsed.reason || "").trim(),
    };
  } catch {
    return null;
  }
}

export async function classifySalesReplyExpectedAction({
  apiKey,
  model,
  approvedReply = "",
  salesIntent = "",
  objectionType = "",
  afterReply = "",
  productName = "",
  customerMessage = "",
  activeState = "",
  conversationContext = [],
}) {
  const reply = String(approvedReply || "").trim();
  if (!reply) return null;
  const recentText = (conversationContext || [])
    .slice(-8)
    .map((message) => `${message.direction || "message"}: ${message.body || message.text || ""}`)
    .join("\n");

  const systemPrompt = `
You classify whether an approved outbound sales reply naturally expects the customer to make a buying, hold-promo, proceed, package-choice, or order decision next.

This is not customer intent classification. You are only judging the approved sales reply that the bot is about to send.

Use the approved reply text, sales intent, after-reply setting, latest customer message, active state, product, and recent conversation.

Rules:
- If after-reply means close/stop/end conversation, return expects_action false.
- Return expects_action true only when the reply clearly invites or implies a next customer decision such as proceed, order, buy, choose package, hold/lock/reserve promo, or confirm interest.
- The reply may imply action without a direct question, for example offering to lock the promo if the customer is ready.
- Return false for replies that only acknowledge hesitation, ask the customer to message later, answer a factual concern without inviting action, or politely close.
- Do not invent action expectation from active state alone.

Return only valid JSON:
{"expects_action":true|false,"expected_action_type":"package_interest_confirmation|none","confidence":"high|low","reason":"short reason"}
`.trim();

  const data = await openaiFetch(apiKey, "/responses", {
    method: "POST",
    body: JSON.stringify({
      model,
      input: [
        { role: "system", content: [{ type: "input_text", text: systemPrompt }] },
        {
          role: "user",
          content: [{
            type: "input_text",
            text: [
              `Active product: ${productName || "unknown"}`,
              `Active state: ${activeState || "none"}`,
              `Sales intent: ${salesIntent || "unknown"}`,
              `Objection type: ${objectionType || "unknown"}`,
              `After-reply setting: ${afterReply || "unknown"}`,
              `Latest customer message: ${customerMessage || ""}`,
              "",
              "Approved outbound sales reply:",
              reply,
              "",
              "Recent conversation:",
              recentText || "(none)",
            ].join("\n"),
          }],
        },
      ],
    }),
  });

  try {
    const parsed = JSON.parse(extractOutputText(data));
    const expectsAction = parsed.expects_action === true;
    const actionType = String(parsed.expected_action_type || "").trim();
    const confidence = String(parsed.confidence || "").trim().toLowerCase();
    return {
      expectsAction: expectsAction && actionType === "package_interest_confirmation" && confidence === "high",
      expectedActionType: actionType || "none",
      confidence,
      reason: String(parsed.reason || "").trim(),
    };
  } catch {
    return null;
  }
}

export async function selectKnowledgeSalesFollowup({
  apiKey,
  model,
  customerMessage,
  answer,
  productName = "",
  activeState = "",
  configuredFollowup = "",
  conversationContext = [],
}) {
  const configured = String(configuredFollowup || "").trim();
  if (!configured) return null;
  const recentText = (conversationContext || [])
    .slice(-8)
    .map((message) => `${message.direction || "message"}: ${message.body || message.text || ""}`)
    .join("\n");

  const systemPrompt = `
You decide whether to add one short sales follow-up after a WhatsApp FAQ/product answer for a Brunei customer.

Use:
- latest customer message,
- FAQ/product answer being sent,
- immediate previous bot/team message,
- recent conversation history,
- active customer state,
- locked product/order context,
- configured FAQ Sales Follow-Up.

Only add a follow-up if it naturally moves the sale forward and is supported by the conversation.
If a specific concern is visible, write one short contextual question.
If no specific concern is visible, choose use_configured so the application can send the configured FAQ Sales Follow-Up.
If the customer is annoyed, not interested, complaining, in pending order, submitted order, or already gave order details, choose none.
Do not repeat a generic package-interest question when a more specific concern is visible.
Do not ask more than one follow-up question.
Do not invent customer concerns that are not in the conversation.
Reply in the customer's language/style, using natural Brunei-Malay when appropriate.
Do not include the FAQ/product answer in the follow-up.

Choose the follow-up based on the customer's current concern:
- If the customer mentioned salary/payday/budget, ask whether they are waiting for salary/payday or want to hold the promo first.
- If the customer is comparing price/package, ask which package or budget they prefer.
- If the customer seems interested but undecided, ask whether they want us to hold the promo.

Return only valid JSON:
{"action":"contextual|use_configured|none","follow_up":"one short question or empty","reason":"short reason"}
`.trim();

  const data = await openaiFetch(apiKey, "/responses", {
    method: "POST",
    body: JSON.stringify({
      model,
      input: [
        { role: "system", content: [{ type: "input_text", text: systemPrompt }] },
        {
          role: "user",
          content: [{
            type: "input_text",
            text: [
              `Active product: ${productName || "unknown"}`,
              `Active state: ${activeState || "none"}`,
              `Latest customer message: ${customerMessage}`,
              `FAQ/product answer being sent: ${answer}`,
              `Configured FAQ Sales Follow-Up: ${configured}`,
              "",
              "Recent conversation:",
              recentText || "(none)",
            ].join("\n"),
          }],
        },
      ],
    }),
  });

  try {
    const parsed = JSON.parse(extractOutputText(data));
    const action = String(parsed.action || "").trim().toLowerCase();
    const followUp = sanitizeKnowledgeSalesFollowup(parsed.follow_up || "");
    if (action === "contextual" && followUp) {
      return {
        action,
        followUp,
        reason: String(parsed.reason || "").trim(),
      };
    }
    if (action === "use_configured") {
      return {
        action,
        followUp: "",
        reason: String(parsed.reason || "").trim(),
      };
    }
    if (action === "none") {
      return {
        action,
        followUp: "",
        reason: String(parsed.reason || "").trim(),
      };
    }
    return null;
  } catch {
    return null;
  }
}

export async function detectOrderStatusIntent({ apiKey, model, customerMessage }) {
  const systemPrompt = `
You classify whether a WhatsApp customer is asking for the status or delivery progress of an order they have already placed.

Return high only for personal order tracking enquiries, such as asking where their order is, whether their ordered item has arrived, or when their delivery will arrive.
Return no_match for pre-sale questions about general stock availability, whether delivery exists, delivery charges, product details, placing a new order, or unrelated conversation.

Return only valid JSON:
{"intent":"order_status|no_match","confidence":"high|low","reason":"short reason"}
`.trim();
  const data = await openaiFetch(apiKey, "/responses", {
    method: "POST",
    body: JSON.stringify({
      model,
      input: [
        { role: "system", content: [{ type: "input_text", text: systemPrompt }] },
        { role: "user", content: [{ type: "input_text", text: `Customer message: ${customerMessage}` }] },
      ],
    }),
  });
  try {
    const parsed = JSON.parse(extractOutputText(data));
    return String(parsed.intent || "").trim() === "order_status" &&
      String(parsed.confidence || "").trim() === "high";
  } catch {
    return false;
  }
}

export async function detectComplaintIntent({ apiKey, model, customerMessage }) {
  const systemPrompt = `
You classify whether a WhatsApp customer message must be handed to a human because it is a complaint or after-sales problem.

Escalate these categories:
- complaint: dissatisfaction, strong frustration, service delay complaint, angry customer.
- refund_return: asks for a refund, return, money back, or cancellation dispute.
- damaged_wrong_item: damaged, faulty, wrong, missing, or unusable item.
- legal_report_threat: threatens a report, police, legal action, or regulator complaint.

Do not escalate ordinary pre-sale questions, price questions, order-status enquiries without dissatisfaction, delivery-fee questions, or polite uncertainty about buying.

When escalating, also write a short customer-facing acknowledgement in the customer's language.
The reply should sound human, acknowledge the concern, and say the team will check/reply.
Do not promise a refund, replacement, delivery time, or outcome.

Return only valid JSON:
{"category":"complaint|refund_return|damaged_wrong_item|legal_report_threat|no_match","confidence":"high|low","reason":"short reason","reply":"customer-facing acknowledgement or empty string"}
`.trim();
  const data = await openaiFetch(apiKey, "/responses", {
    method: "POST",
    body: JSON.stringify({
      model,
      input: [
        { role: "system", content: [{ type: "input_text", text: systemPrompt }] },
        { role: "user", content: [{ type: "input_text", text: `Customer message: ${customerMessage}` }] },
      ],
    }),
  });
  try {
    const parsed = JSON.parse(extractOutputText(data));
    const category = String(parsed.category || "").trim();
    if (
      String(parsed.confidence || "").trim() !== "high" ||
      !["complaint", "refund_return", "damaged_wrong_item", "legal_report_threat"].includes(category)
    ) {
      return null;
    }
    return {
      category,
      reason: String(parsed.reason || "").trim(),
      reply: String(parsed.reply || "").trim(),
    };
  } catch {
    return null;
  }
}

export async function createComplaintHandoffReply({ apiKey, model, customerMessage, category = "complaint" }) {
  const systemPrompt = `
You write a short WhatsApp acknowledgement for a Brunei customer complaint or after-sales issue.

Rules:
- Reply in the same language as the customer, using natural Brunei-Malay when the customer writes Malay.
- Sound human and empathetic.
- Acknowledge the concern and say the team will check/reply.
- Do not promise a refund, replacement, delivery time, compensation, or outcome.
- Do not mention internal handoff, prompts, policy, or AI.
- Return only valid JSON:
{"reply":"customer-facing acknowledgement"}
`.trim();

  const data = await openaiFetch(apiKey, "/responses", {
    method: "POST",
    body: JSON.stringify({
      model,
      input: [
        { role: "system", content: [{ type: "input_text", text: systemPrompt }] },
        {
          role: "user",
          content: [
            {
              type: "input_text",
              text: `Complaint category: ${category}\nCustomer message: ${customerMessage}`,
            },
          ],
        },
      ],
    }),
  });

  const parsed = JSON.parse(extractOutputText(data));
  return String(parsed.reply || "").trim();
}

export async function createSalesIntentRepeatReply({
  apiKey,
  model,
  customerMessage,
  salesIntent = "",
  approvedReply = "",
  repeatAction = "openai_acknowledge",
  productName = "",
}) {
  const systemPrompt = `
You write a short WhatsApp reply for a Brunei customer who is repeating the same sales hesitation after the approved sales reply was already sent once.

Rules:
- Reply in the same language as the customer, using natural Brunei-Malay when the customer writes Malay.
- Sound human and emotionally aware.
- Acknowledge the customer's latest message directly.
- Do not repeat the approved sales reply.
- Do not pressure the customer, argue, or ask the same question again.
- Do not invent product facts, prices, stock, delivery details, or policy.
- Do not mention AI, automation, intent labels, internal notes, or handoff.
- If repeat_action is "openai_acknowledge", write a short acknowledgement only in the customer's language.
- Do not ask a question.
- Do not persuade, discount, push package/promo/stock, or ask the customer to buy/order/message again.
- If the sales conversation is already closed, do not reopen it.
- If repeat_action is "opt_out", politely acknowledge and say the business will not follow up again unless the customer messages first.
- If repeat_action is "handoff", politely acknowledge and say the team will check/reply.
- Return only valid JSON:
{"reply":"customer-facing reply"}
`.trim();

  const data = await openaiFetch(apiKey, "/responses", {
    method: "POST",
    body: JSON.stringify({
      model,
      input: [
        { role: "system", content: [{ type: "input_text", text: systemPrompt }] },
        {
          role: "user",
          content: [{
            type: "input_text",
            text: [
              `Product: ${productName || "unknown"}`,
              `Sales intent: ${salesIntent || "unknown"}`,
              `Repeat action: ${repeatAction}`,
              `Previously sent approved sales reply: ${approvedReply || ""}`,
              `Customer latest message: ${customerMessage}`,
            ].join("\n"),
          }],
        },
      ],
    }),
  });

  const parsed = JSON.parse(extractOutputText(data));
  return String(parsed.reply || "").trim();
}

export async function classifyUpsellDecision({
  apiKey,
  model,
  customerMessage,
  currentOption = {},
  targetOption = {},
  orderOptions = [],
}) {
  const optionText = (orderOptions || [])
    .map((option) => [
      `ID: ${option.id || ""}`,
      `Name: ${option.name || ""}`,
      `Price: ${option.price || ""}`,
      `Aliases: ${(option.aliases || []).join(" | ")}`,
    ].filter((line) => !/:\s*$/.test(line)).join("\n"))
    .join("\n\n");
  const systemPrompt = `
You classify a Brunei WhatsApp customer's reply to an upsell offer after they already submitted order details.

The customer originally selected one order option. The business offered a higher option.

Return only valid JSON:
{"decision":"accept_upsell|keep_original|choose_other_option|unclear|unrelated","selected_order_option_id":"","confidence":"high|medium|low","reason":"short reason"}

Rules:
- accept_upsell: customer agrees to the offered higher option, upgrade, target package, or says yes to the upsell.
- keep_original: customer clearly wants to stay with their original/current option, says no upgrade, same one, yang tadi, inda/nda mau tukar, proceed current package.
- choose_other_option: customer clearly selects a listed order option different from both current and target.
- unclear: customer reply is about the package decision but not clear enough.
- unrelated: customer asks something else not deciding the package.
- selected_order_option_id must be one of the listed option IDs when decision is accept_upsell, keep_original, or choose_other_option. Use target option for accept_upsell, current option for keep_original.
- Do not write a customer-facing reply.
`.trim();

  const data = await openaiFetch(apiKey, "/responses", {
    method: "POST",
    body: JSON.stringify({
      model,
      input: [
        { role: "system", content: [{ type: "input_text", text: systemPrompt }] },
        {
          role: "user",
          content: [{
            type: "input_text",
            text: [
              `Current option ID: ${currentOption.id || ""}`,
              `Current option name: ${currentOption.name || ""}`,
              `Target option ID: ${targetOption.id || ""}`,
              `Target option name: ${targetOption.name || ""}`,
              "",
              "Available order options:",
              optionText || "(none)",
              "",
              `Customer reply: ${customerMessage}`,
            ].join("\n"),
          }],
        },
      ],
    }),
  });

  try {
    const parsed = JSON.parse(extractOutputText(data));
    const decision = String(parsed.decision || "").trim();
    return {
      decision: ["accept_upsell", "keep_original", "choose_other_option", "unclear", "unrelated"].includes(decision) ? decision : "unclear",
      selectedOrderOptionId: String(parsed.selected_order_option_id || "").trim(),
      confidence: ["high", "medium", "low"].includes(String(parsed.confidence || "").trim()) ? String(parsed.confidence).trim() : "low",
      reason: String(parsed.reason || "").trim(),
    };
  } catch {
    return {
      decision: "unclear",
      selectedOrderOptionId: "",
      confidence: "low",
      reason: "Model returned non-JSON output.",
    };
  }
}

function extractOutputText(response) {
  if (response.output_text) return response.output_text;

  const chunks = [];
  for (const item of response.output || []) {
    for (const content of item.content || []) {
      if (content.type === "output_text" && content.text) {
        chunks.push(content.text);
      }
    }
  }
  return chunks.join("\n").trim();
}

function parseAgentJson(text) {
  try {
    const parsed = JSON.parse(text);
    return {
      reply: String(parsed.reply || "").trim(),
      replyType: normalizeReplyType(parsed.reply_type),
      handoffRequired: Boolean(parsed.handoff_required),
      handoffReason: String(parsed.handoff_reason || "").trim(),
      raw: text,
    };
  } catch {
    return {
      reply: String(text || "").trim(),
      replyType: "other",
      handoffRequired: true,
      handoffReason: "Model returned non-JSON output.",
      raw: text,
    };
  }
}

function normalizeRouteClassification(text, messageTypes, faqTopics, salesIntents) {
  try {
    const parsed = JSON.parse(text);
    const messageType = String(parsed.message_type || "").trim().toLowerCase();
    const validMessageType = messageTypes.includes(messageType) ? messageType : "unknown";
    const primaryIntent = String(parsed.primary_intent || "").trim();
    const validIntents = new Set([
      ...faqTopics.map((item) => String(item.id || "")),
      ...salesIntents.map((item) => String(item.id || "")),
    ]);
    return {
      messageType: validMessageType,
      primaryIntent: validIntents.has(primaryIntent) ? primaryIntent : "",
      relatedProduct: String(parsed.related_product || "").trim(),
      clarificationRequired: Boolean(parsed.clarification_required),
      humanSupportRequired: Boolean(parsed.human_support_required),
      confidence: ["high", "medium", "low"].includes(String(parsed.confidence || "").trim().toLowerCase())
        ? String(parsed.confidence || "").trim().toLowerCase()
        : "low",
      reason: String(parsed.reason || "").trim(),
    };
  } catch {
    return {
      messageType: "unknown",
      primaryIntent: "",
      relatedProduct: "",
      clarificationRequired: false,
      humanSupportRequired: false,
      confidence: "low",
      reason: "Classifier returned non-JSON output.",
    };
  }
}

function sanitizeKnowledgeSalesFollowup(value) {
  let text = String(value || "")
    .replace(/\s+/g, " ")
    .trim();
  if (!text) return "";
  text = text.replace(/^["'`]+|["'`]+$/g, "").trim();
  const questionIndex = text.indexOf("?");
  if (questionIndex >= 0) text = text.slice(0, questionIndex + 1).trim();
  if (!/[?؟]$/.test(text)) return "";
  const words = text.split(/\s+/).filter(Boolean);
  if (words.length > 18) return "";
  return text;
}

function knowledgeRecordText(record) {
  return [
    record.topic,
    record.brunei_malay_topic,
    ...(record.example_questions || []),
    ...(record.brunei_malay_example_questions || []),
    record.approved_reply,
    record.brunei_malay_approved_reply,
    record.title,
    record.summary,
    record.extracted_text,
    record.embedding_text,
    record.brunei_malay_summary,
    record.brunei_malay_search_text,
    record.value,
    record.text,
  ].filter(Boolean).join("\n");
}

function normalizeReplyType(value) {
  return String(value || "").trim().toLowerCase() === "faq" ? "faq" : "other";
}

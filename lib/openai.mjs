...
- OpenAI vector-store knowledge containing approved general FAQ, approved product FAQ, approved extracted product image knowledge, and approved sales replies
...
- For this customer-service answer call, ignore sales reply records in the vector store. Approved sales replies are selected by a separate sales-reply selector before this call.
...
Choose a SALES_REPLY_ID only when the customer clearly matches one listed SALES_INTENT and the listed examples/intent directly fit the customer's message.
The approved reply will be sent by the application, so do not write a customer reply.
...
Use the product-specific records only for ${productName}; general records apply to any product.
Do not use outside knowledge, do not choose the closest broad hesitation, and do not infer a sales objection from unrelated wording.
...
export async function selectSalesReplyFromVectorStore({
  apiKey,
  model,
  vectorStoreId,
  customerMessage,
  ...
  const systemPrompt = `
You select an approved WhatsApp sales reply for Brunei customers.

Use file_search as the source of truth.
Only use records from "Vector Store Knowledge: Approved Sales Replies".
Ignore general FAQ, product FAQ, product image knowledge, SOP, reply flows, and unapproved notes.

Choose a SALES_REPLY_ID only when the customer clearly matches one approved SALES_INTENT from the retrieved records and the examples/intent directly fit the customer's message.
Do not match factual product questions, delivery questions, price questions, order-detail submissions, order status enquiries, opt-outs, complaints, refund/damage issues, legal/report threats, or unrelated chat.
Do not choose the closest broad hesitation if the retrieved record does not directly fit.
The application will send the saved approved reply, so do not write a reply.
If no approved sales reply directly fits, return no_match.
If uncertain, return no_match.

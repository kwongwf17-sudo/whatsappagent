...
- Also create Brunei-Malay customer wording for the same knowledge:
  - brunei_malay_summary: translate or lightly paraphrase only the image_chunk.summary and visible extracted text into natural Brunei/Malay.
  - brunei_malay_search_text: include only Brunei/Malay equivalents of words and facts already present in image_chunk.summary, extracted_text, or embedding_text.
  - brunei_malay_question_examples: example customer questions only for facts visibly present in this exact image.
- Do not create broad customer-search keywords, likely FAQ topics, or sales keywords that are not direct translations/equivalents of visible image content.

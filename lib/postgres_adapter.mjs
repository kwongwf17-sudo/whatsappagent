...
export class PostgresJsonAdapter {
  ...
  async deleteJsonDocumentKey(key) {
    await this.ready;
    const result = await this.pool.query(`DELETE FROM ${this.tableName} WHERE key = $1`, [String(key || "")]);
    return result.rowCount || 0;
  }

  async deleteJsonDocumentKeysByPrefix(prefix = "") {
    await this.ready;
    const result = await this.pool.query(`DELETE FROM ${this.tableName} WHERE key LIKE $1`, [`${String(prefix || "")}%`]);
    return result.rowCount || 0;
  }
  ...
  async deleteOutboxForAccount(businessAccountId = "") {
    await this.ready;
    if (!this.outboxTableEnabled) return 0;
    const accountId = String(businessAccountId || "").trim();
    if (!accountId) throw new Error("Business account ID is required.");
    const result = await this.pool.query(
      `DELETE FROM ${this.outboxTableName} WHERE business_account_id = $1`,
      [accountId]
    );
    return result.rowCount || 0;
  }
  ...
  async deleteCustomerRowsForAccount(businessAccountId = "") {
    await this.ready;
    if (!this.customersTableEnabled) return 0;
    const accountId = String(businessAccountId || "").trim();
    if (!accountId) throw new Error("Business account ID is required.");
    const result = await this.pool.query(
      `DELETE FROM ${this.customersTableName} WHERE business_account_id = $1`,
      [accountId]
    );
    return result.rowCount || 0;
  }
  ...
  async deleteOrderRowsForAccount(businessAccountId = "") {
    await this.ready;
    if (!this.ordersTableEnabled) return 0;
    const accountId = String(businessAccountId || "").trim();
    if (!accountId) throw new Error("Business account ID is required.");
    const result = await this.pool.query(
      `DELETE FROM ${this.ordersTableName} WHERE business_account_id = $1`,
      [accountId]
    );
    return result.rowCount || 0;
  }
  ...
  async importProcessedMessageRows(messages = {}, { overwrite = false, retentionMs = 0 } = {}) {
    ...
    let count = 0;
    for (const batch of chunks(records.filter((record) => record?.messageId), 500)) {
      const values = [];
      const placeholders = batch.map((record, index) => {
        ...
        return `($${offset + 1}, $${offset + 2}, $${offset + 3}::timestamptz, $${offset + 4}::timestamptz, $${offset + 5}, $${offset + 6}, $${offset + 7}::jsonb)`;
      });
      const result = await this.pool.query(
        `
          INSERT INTO ${this.processedMessagesTableName} (
  ...
  async deleteProcessedMessageRowsForAccount(businessAccountId = "") {
    await this.ready;
    if (!this.processedMessagesTableEnabled) return 0;
    const accountId = String(businessAccountId || "").trim();
    if (!accountId) throw new Error("Business account ID is required.");
    const result = await this.pool.query(
      `DELETE FROM ${this.processedMessagesTableName} WHERE business_account_id = $1`,
      [accountId]
    );
    return result.rowCount || 0;
  }
  ...
  async deletePendingBufferRowsForAccount(businessAccountId = "") {
    await this.ready;
    if (!this.pendingBuffersTableEnabled) return 0;
    const accountId = String(businessAccountId || "").trim();
    if (!accountId) throw new Error("Business account ID is required.");
    const result = await this.pool.query(
      `DELETE FROM ${this.pendingBuffersTableName} WHERE business_account_id = $1`,
      [accountId]
    );
    return result.rowCount || 0;
  }
  ...
  async deleteAuditEventsForAccount(accountId = "") {
    await this.ready;
    if (!this.auditEventsTableEnabled) return 0;
    ...
    const result = await this.pool.query(
      `
        DELETE FROM ${this.auditEventsTableName}
        WHERE account_id = $1
          OR payload->>'businessAccountId' = $1
          OR payload->>'accountId' = $1
          OR payload->>'result' = $1
          OR payload->>'actor' = $2
          OR payload->>'actor' = $3
      `,
      [id, `admin:${id}`, `business_admin:${id}`]
    );
    return result.rowCount || 0;
  }
  ...
  async deleteOperationalEventsForAccount(businessAccountId = "") {
    await this.ready;
    if (!this.operationalEventsTableEnabled) return 0;
    const accountId = String(businessAccountId || "").trim();
    if (!accountId) throw new Error("Business account ID is required.");
    const result = await this.pool.query(
      `DELETE FROM ${this.operationalEventsTableName} WHERE business_account_id = $1`,
      [accountId]
    );
    return result.rowCount || 0;
  }
  ...
  async deleteTeamContentRecord(accountId) {
    await this.ready;
    if (!this.teamContentTableEnabled) return 0;
    const key = String(accountId || "").trim();
    if (!key) throw new Error("Account ID is required.");
    const products = await this.pool.query(
      `DELETE FROM ${this.teamContentProductsTableName} WHERE business_account_id = $1`,
      [key]
    );
    const account = await this.pool.query(
      `DELETE FROM ${this.teamContentAccountsTableName} WHERE business_account_id = $1`,
      [key]
    );
    return (products.rowCount || 0) + (account.rowCount || 0);
  }
  ...
  async deleteFollowupDispatchesForAccount(businessAccountId = "") {
    await this.ready;
    if (!this.followupQueueTableEnabled) return 0;
    const accountId = String(businessAccountId || "").trim();
    if (!accountId) throw new Error("Business account ID is required.");
    const result = await this.pool.query(
      `DELETE FROM ${this.followupQueueTableName} WHERE business_account_id = $1`,
      [accountId]
    );
    return result.rowCount || 0;
  }

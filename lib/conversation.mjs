...
export async function buildConversationPlan({
  ...
  if (salesReply) {
    return salesReplyPlan(customer, product, salesReply);
  }
...
function salesReplyFromMatch(match = {}) {
  return {
    ...
    followup_prompt: match.followupPrompt || "",
    cooldown_hours: match.cooldownHours,
    max_sends_per_customer: match.maxSendsPerCustomer,
  };
}

function salesReplyPlan(customer, product, salesReply) {
  const decision = salesReplyDeliveryDecision(customer, product, salesReply);
  if (!decision.allowed) {
    return {
      customerPatch: {
        productId: product.id,
        awaitingPackageBInterest: false,
        lastSalesReplySuppressedId: salesReply.id,
        lastSalesReplySuppressedIntent: decision.intentKey,
        lastSalesReplySuppressedAt: decision.nowIso,
        lastSalesReplySuppressedReason: decision.reason,
      },
      messages: [],
      handoffRequired: false,
    };
  ...
  return {
    customerPatch: {
      ...
      lastSalesReplyIntent: decision.intentKey,
      lastSalesReplyAt: decision.nowIso,
      salesReplyState: nextSalesReplyState(customer, product, salesReply, decision),
    },
...
function salesReplyDeliveryDecision(customer, product, salesReply, now = new Date()) {
  const nowIso = now.toISOString();
  const intentKey = salesReplyIntentKey(salesReply);
  const state = salesReplyIntentState(customer, product, intentKey);
  const maxSends = salesReplyMaxSends(salesReply);
  const cooldownHours = salesReplyCooldownHours(salesReply);
  if (maxSends > 0 && Number(state.count || 0) >= maxSends) {
    return { allowed: false, reason: "max_sends_reached", intentKey, state, maxSends, cooldownHours, now, nowIso };
  }
  const cooldownUntil = state.cooldownUntil ? new Date(state.cooldownUntil) : null;
  if (cooldownUntil && Number.isFinite(cooldownUntil.getTime()) && cooldownUntil > now) {
    return { allowed: false, reason: "cooldown_active", intentKey, state, maxSends, cooldownHours, now, nowIso };
  }
  return { allowed: true, reason: "", intentKey, state, maxSends, cooldownHours, now, nowIso };
}

function nextSalesReplyState(customer, product, salesReply, decision) {
  const productKey = productSalesReplyStateKey(product);
  const intentKey = decision.intentKey;
  const current = salesReplyIntentState(customer, product, intentKey);
  const nextCount = Number(current.count || 0) + 1;
  const cooldownUntil = decision.cooldownHours > 0
    ? new Date(decision.now.getTime() + decision.cooldownHours * 60 * 60 * 1000).toISOString()
    : "";
  return {
    ...(customer?.salesReplyState && typeof customer.salesReplyState === "object" ? customer.salesReplyState : {}),
    [productKey]: {
      ...(customer?.salesReplyState?.[productKey] && typeof customer.salesReplyState[productKey] === "object"
        ? customer.salesReplyState[productKey]
        : {}),
      [intentKey]: {
        count: nextCount,
        lastReplyId: salesReply.id || "",
        lastSentAt: decision.nowIso,
        cooldownUntil,
      },
    },
  };
...
function salesReplyIntentState(customer, product, intentKey) {
  const productKey = productSalesReplyStateKey(product);
  return customer?.salesReplyState?.[productKey]?.[intentKey] || {};
}

function productSalesReplyStateKey(product) {
  return String(product?.id || "global").trim() || "global";
}
...
function salesReplyCooldownHours(salesReply = {}) {
  const value = Number(salesReply.cooldown_hours ?? salesReply.cooldownHours ?? DEFAULT_SALES_REPLY_COOLDOWN_HOURS);
  return Number.isFinite(value) && value >= 0 ? value : DEFAULT_SALES_REPLY_COOLDOWN_HOURS;
}

function salesReplyMaxSends(salesReply = {}) {
  const value = Number(salesReply.max_sends_per_customer ?? salesReply.maxSendsPerCustomer ?? DEFAULT_SALES_REPLY_MAX_SENDS);
  return Number.isFinite(value) && value >= 0 ? Math.trunc(value) : DEFAULT_SALES_REPLY_MAX_SENDS;
}

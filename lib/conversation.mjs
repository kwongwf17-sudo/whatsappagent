...
export async function buildConversationPlan({
  ...
  const forceRagKnowledge = routeForcesRagKnowledge(routeClassification);
  ...
  const salesReply = allowLocalSales || hasSalesObjectionLanguage(text)
  ...
  if (productOpeningFlow || productNameOpening) {
    return {
      customerPatch: {
        productId: product.id,
  ...
  const approvedFaq = !allowLocalKnowledge || forceRagKnowledge
    ? null
    : ragAnswer
  ...
  const productPriceReply = allowLocalKnowledge && !forceRagKnowledge
  ...
  const faqAnswer = allowLocalKnowledge && !forceRagKnowledge
...
function routeForcesRagKnowledge(route) {
  return Boolean(route && route.confidence !== "low" && ["general_faq", "product_question"].includes(route.messageType));
}
...
function shouldAnswerBeforePendingOrder(customer, text, currentDraft, orderDraft, product, routeClassification = null) {
  ...
  if (currentDraft?.isComplete || currentDraft?.hasAnyDetails || orderDraft?.hasNewDetails) return false;
  ...
  if (routeClassification && routeClassification.confidence !== "low") {
    return ["general_faq", "product_question", "sales_reply", "order_status", "complaint", "human_request", "unknown"].includes(
      routeClassification.messageType
    );
  }
...
function shouldHandleOrderDraft(customer, text, orderDraft, product, ragAnswer, routeClassification = null) {
  ...
  if (customer?.pendingOrder) {
    return Boolean(orderDraft.isComplete || orderDraft.hasNewDetails || requestsCurrentChatPhone(text) || isOrderStartIntent(text) || hasPurchaseIntent);
  }

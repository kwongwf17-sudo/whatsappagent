...
export async function buildConversationPlan({
  ...
  const earlyOrderDraft = extractOrderDetails(text, product);
  if (earlyOrderDraft.isComplete || hasOrderFormFields(earlyOrderDraft) || isOrderStartIntent(text) || ragAnswer?.buyingIntent === "buying") {
    if (earlyOrderDraft.isComplete) {
      return {
        ...
        messages: [
          textMessage(
            "Sorry Dear our stock just finish , it will take order again, will take around 15-18 days for arrived brunei new stock ðŸ¥°\n\nREMINDER : ORDER AFTER 1 HOURS CANNOT BE CANCEL"
          ),
          textMessage(
            "But i will proceed system for COD service ðŸ¥°\n\nBrg Sampai baru byr runner"
          ),
        ],
  ...
  if (approvedFaq) {
    return {
      ...
      messages: faqReplyMessages(approvedFaq.approved_reply, product),
      handoffRequired: false,
    };
  }
  ...
  if (productPriceReply) {
    return {
      ...
      messages: faqReplyMessages(productPriceReply.approved_reply, product),
      handoffRequired: false,
    };
  }

  if (productFactMatch?.reply) {
    return {
      customerPatch: {
        productId: product.id,
        awaitingPackageBInterest: true,
        lastProductFactId: productFactMatch.factId,
      },
      messages: faqReplyMessages(productFactMatch.reply, product),
      handoffRequired: false,
    };
  }

  const productFact = findApprovedProductFact(product, text);
  if (productFact) {
    return {
      customerPatch: {
        productId: product.id,
        awaitingPackageBInterest: true,
        lastProductFactId: productFact.id,
      },
      messages: faqReplyMessages(productFact.reply, product),
      handoffRequired: false,
    };
  }
  ...
  if (standardReply) {
    return {
      customerPatch: {
        productId: product.id,
        awaitingPackageBInterest: standardReply.type === "faq",
      },
      messages:
        standardReply.type === "faq"
          ? faqReplyMessages(standardReply.reply, product)
  ...
  const orderDraft = extractOrderDetails(text, product);
  if (orderDraft.isComplete || hasOrderFormFields(orderDraft) || isOrderStartIntent(text) || ragAnswer?.buyingIntent === "buying") {
  ...
  if (approvedFaqMatch?.approvedReply) {
    return {
      ...
      messages: faqReplyMessages(approvedFaqMatch.approvedReply, product),
      handoffRequired: false,
    };
  }
  ...
  if (faqAnswer) {
    return {
      customerPatch: { productId: product.id, awaitingPackageBInterest: true },
      messages: faqReplyMessages(faqAnswer, product),
      handoffRequired: false,
    };
  }

  if (ragAnswer?.reply && !productSpecificQuestion) {
    return {
      customerPatch: {
        productId: product.id,
        awaitingPackageBInterest: ragAnswer.replyType === "faq" && !ragAnswer.handoffRequired,
      },
      messages:
        ragAnswer.replyType === "faq" && !ragAnswer.handoffRequired
          ? faqReplyMessages(ragAnswer.reply, product)
...
export function findProduct(catalog, text = "", source = {}, fallbackProductId = "") {
  ...
  const byMatch = products.find((product) => {
    const terms = [product.name, product.id, ...(product.aliases || []), ...(product.ad_keywords || [])];
...
export function isProductNameMessage(product, text) {
  const normalized = normalizeReplyText(text);
  if (!normalized) return false;
  const terms = [product.name, product.id, ...(product.aliases || [])]
...
export function approvedProductFactRecordsForProduct(product) {
  ...
  const chunks = (product?.extracted_knowledge?.approvedImages || [])
    ...
    .map((chunk, index) => ({
      ...chunk,
      kind: "image_chunk",
      id: chunk.id || `${product.id}_image_${index + 1}`,
      product_id: product.id,
      product_name: product.name,
      label: chunk.title || "Image knowledge",
      value: chunk.brunei_malay_summary || chunk.summary || chunk.extracted_text || chunk.embedding_text,
      approved_reply: formatImageChunkReply(chunk),
...
export function findSalesReplyExactMatch(catalog, product, text, options = {}) {
  ...
  return (
    salesReplyRecordsForProduct(catalog, product, options).find((item) =>
      (item.example_messages || []).some((message) => normalizeReplyText(message) === normalizedText)
    ) || null
  );
}
...
function findApprovedProductFact(product, text) {
  const facts = approvedProductFactRecordsForProduct(product);
  if (!facts.length) return null;
  const normalized = normalizeReplyText(text);
  if (isBusinessOrLogisticsQuestion(normalized) && !/\b(cod|refund|warranty|rosak)\b/i.test(normalized)) return null;
  if (isUsageDurationQuestion(normalized)) return null;
  if (isProductOriginQuestion(normalized)) return null;
  const mlQuestion = /\b(ml|milliliter|millilitre|volume|isi|berapa\s*ml|berapa\s*mililiter)\b/i.test(normalized);
  for (const fact of facts) {
    const haystack = [
      ...
      fact.extracted_text,
      fact.embedding_text,
      fact.brunei_malay_summary,
      fact.brunei_malay_search_text,
      ...(fact.question_examples || []),
      ...(fact.brunei_malay_question_examples || []),
    ].map((item) => normalizeReplyText(item)).join(" ");
    const factIsVolume = /\b(ml|milliliter|millilitre|volume|isi)\b/i.test(haystack);
    if (mlQuestion && factIsVolume) {
      return {
        id: fact.id,
        reply: formatProductFactReply(fact, normalized),
      };
    }
    if (fact.kind === "image_chunk") {
      if ((fact.question_examples || []).some((question) => normalizeReplyText(question) === normalized)) {
        return {
          id: fact.id,
          reply: formatProductFactReply(fact, normalized),
        };
      }
      if (imageChunkLocalMatch(normalized, haystack, fact)) {
        return {
          id: fact.id,
          reply: formatProductFactReply(fact, normalized),
        };
    ...
    const labelTerms = normalizeReplyText(`${fact.label || ""} ${fact.category || ""}`).split(/[^a-z0-9]+/).filter((term) => term.length > 2);
    if (labelTerms.length && labelTerms.some((term) => normalized.includes(term))) {
      return {
        id: fact.id,
        reply: formatProductFactReply(fact, normalized),
      };
...
function isBusinessOrLogisticsQuestion(normalizedText) {
  return /\b(area|location|lokasi|alamat|warehouse|kedai|store|shop|delivery|deliver|hantar|perhantaran|penghantaran|shipping|cod|bayar|payment|caj|charge|fee)\b/i.test(normalizedText);
}
...
function imageChunkLocalMatch(normalizedQuestion, normalizedKnowledge, fact) {
  const category = String(fact.category || "").toLowerCase();
  if (/\bcod\b/i.test(normalizedQuestion) && /\bcod\b/i.test(normalizedKnowledge)) return true;
  if (/\b(refund|warranty|rosak)\b/i.test(normalizedQuestion) && /\b(refund|warranty|rosak)\b/i.test(normalizedKnowledge)) return true;
  if (category === "feature" && featureQuestionIntent(normalizedQuestion, normalizedKnowledge)) return true;
  if (category === "price" && /\b(add[\s-]?on|combo|package|pakej)\b/i.test(normalizedQuestion)) {
    return meaningfulOverlap(normalizedQuestion, normalizedKnowledge) >= 1;
  }
  return false;
}

function featureQuestionIntent(normalizedQuestion, normalizedKnowledge = "") {
  if (!normalizedQuestion) return "";
  const hasKnowledge = (pattern) => pattern.test(normalizedKnowledge);
  if (/\b(recharge|rechargeable|charging|charge|cas|bateri|battery|usb)\b/i.test(normalizedQuestion) && hasKnowledge(/\b(recharge|rechargeable|charging|usb|usb-c|cas)\b/i)) {
    return "recharge";
  }
  if (/\b(mode|modes|level|intensity|normal|intermediate|strong)\b/i.test(normalizedQuestion) && hasKnowledge(/\b(mode|modes|normal|intermediate|strong|intensity)\b/i)) {
    return "mode";
  }
  if (/\b(suction|sedut|sedutan|kuat|kpa)\b/i.test(normalizedQuestion) && hasKnowledge(/\b(suction|sedut|sedutan|strong|kuat|kpa|60kpa)\b/i)) {
    return "suction";
  }
  if (/\b(head|heads|kepala|probe)\b/i.test(normalizedQuestion) && hasKnowledge(/\b(head|heads|kepala|probe)\b/i)) {
    return "head";
  }
  if (/\b(fungsi|function|benefit|kegunaan|untuk\s*apa|guna\s*apa)\b/i.test(normalizedQuestion) && hasKnowledge(/\b(blackhead|pore|pores|minyak|oil|suction|sedut|clean)\b/i)) {
    return "function";
  }
  return "";
}

function formatProductFactReply(fact, normalizedQuestion = "") {
  if (fact.kind === "image_chunk") return formatImageChunkReply(fact, normalizedQuestion);
  const label = String(fact.label || "").trim();
  const value = String(fact.value || "").trim();
  if (!label) return value;
  if (!value || normalizeReplyText(label) === normalizeReplyText(value)) return label;
  return `${label}: ${value}`;
}
...
function formatImageChunkReply(chunk, normalizedQuestion = "") {
  ...
  if (category === "feature" && text) {
    const intent = featureQuestionIntent(normalizedQuestion, normalizeReplyText(fullText));
    if (intent === "recharge") {
      return "Boleh recharge ya. Blackhead Remover ani USB-C rechargeable, senang cas dan mudah bawa.";
    }
    if (intent === "mode") {
      return "Ada 3 suction modes: Normal, Intermediate dan Strong. Pilih ikut kesesuaian kulit. Untuk kulit sensitif, mula dari Normal dulu.";
    }
    if (intent === "suction") {
      return "Suction kuat ya, 60KPA strong suction. Ia bantu tarik keluar blackhead, minyak dan kotoran pori.";
    }
    if (intent === "head") {
      return "Ada 3 suction heads untuk kawasan berbeza seperti hidung, dagu, pipi dan T-zone.";
    }
    if (intent === "function") {
      return "Fungsinya bantu sedut blackhead, minyak dan kotoran pori supaya kawasan muka nampak lebih bersih.";
    }
    return text
      .replace(/\bThe product shows\b/i, "Produk ani ada")
      .replace(/\.$/, ".")
      .trim();
  }
  if (category === "price") {
...
function meaningfulOverlap(normalizedQuestion, normalizedKnowledge) {
  const questionTerms = normalizedQuestion
    .split(/[^a-z0-9]+/)
    .filter((term) => term.length > 3 && !FAQ_STOPWORDS.has(term) && !GENERIC_MATCH_STOPWORDS.has(term));
  if (!questionTerms.length) return 0;
  return questionTerms.filter((term) => normalizedKnowledge.includes(term)).length;
}
...
function faqReplyMessages(answer, product) {
  return [textMessage(answer), textMessage(salesPromptForProduct(product))];
}
...
export function extractOrderDetails(text, product) {
  ...
  const name = fieldValue(normalizedText, ["full name", "name", "nama"]);
  const phone = fieldValue(normalizedText, ["phone number", "phone", "tel", "mobile", "whatsapp"]);
  const address = fieldValue(normalizedText, ["full address", "delivery address", "address", "alamat"]);

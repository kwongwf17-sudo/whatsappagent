...
const baseDecisionRules = [
  ...
  },
  {
    ...
    blockedStates: ["complaint", "handoff", "submittedOrder", "done", "optedOut", "anotherDatePurchase"],
    resolve: ({ product, customer, text, conversationContext }) => buildClarificationPlan({
      product,
      customer,
      text,
      conversationContext,
...
export function classifyFaqSalesPromptResponse(customer, customerMessage) {
  ...
  const normalized = normalizeReplyText(customerMessage);
  if (/^(ada|awu|awu ada|ya|yes|ok|okay|okey|k|kk|boleh|buleh|can|jadi|mau|nak|mahu|ambil|minat)$/.test(normalized)) {
    return "interested";
  }
  return "";
}

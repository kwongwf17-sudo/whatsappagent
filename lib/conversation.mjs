...
export async function buildConversationPlan({
  ...
  if (shouldHandleOrderDraft(customer, text, earlyOrderDraft, product, ragAnswer)) {
  ...
  if (shouldHandleOrderDraft(customer, text, orderDraft, product, ragAnswer)) {
...
function shouldHandleOrderDraft(customer, text, orderDraft, product, ragAnswer) {
  if (customer?.pendingOrder) {
    return Boolean(orderDraft.isComplete || orderDraft.hasNewDetails || hasOrderFormFields(orderDraft) || isOrderStartIntent(text) || ragAnswer?.buyingIntent === "buying");
  }
  if (isOrderStartIntent(text) || ragAnswer?.buyingIntent === "buying") return true;

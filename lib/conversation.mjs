...
function parseMixedLineOrderDetails(line) {
  ...
  const addressMatch = cleaned.match(/\b(no\.?|simpang|spg|jalan|jln|kg|kpg|kampong|kampung|stkrj|rumah|house|unit|block|blk|lot|mumong|kiulap|tutong|kb|kuala\s*belait|temburong|brunei|bandar|seria|jerudong|rimba|lambak|berakas|mengkubau|mulaut|sengkurong|lumut|liang)\b/i);
  if (!addressMatch) {
    return { phone };
  }
  const matchedAddressCue = String(addressMatch[1] || "").toLowerCase();
  const hasStrongAddressCue = /\b(no\.?|simpang|spg|jalan|jln|kg|kpg|kampong|kampung|stkrj|rumah|house|block|blk|lot|mumong|kiulap|tutong|kb|kuala\s*belait|temburong|brunei|bandar|seria|jerudong|rimba|lambak|berakas|mengkubau|mulaut|sengkurong|lumut|liang)\b/i.test(cleaned);
...
function isLikelyAddressLine(line) {
  const text = String(line || "").trim();
  if (!text) return false;
  const hasAddressKeyword = /\b(no\.?|simpang|spg|jalan|jln|kg|kpg|kampong|kampung|stkrj|rumah|house|unit|block|blk|lot|mumong|kiulap|tutong|kb|kuala\s*belait|temburong|brunei|bandar|seria|jerudong|rimba|lambak|berakas|mengkubau|mulaut|sengkurong|lumut|liang)\b/i.test(text);

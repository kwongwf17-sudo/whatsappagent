...
const PRODUCT_CONTEXT_CLARIFICATION_REPLY =
  "Hi, boleh saya tahu produk mana yang kita berminat? Kami belum dapat nampak produk dari chat ani, jadi saya mau pastikan saya share info yang betul ya 🙏";
...
const baseDecisionRules = [
  ...
  },
  {
    ...
    blockedStates: ["complaint", "handoff", "submittedOrder", "done", "optedOut", "anotherDatePurchase"],
    resolve: ({ customer, productResolution, productSpecificQuestion }) => buildProductContextClarificationPlan({
      customer,
      productResolution,
      productSpecificQuestion,
    }),
  },
  {
    ...
    description: "Answer approved FAQ, product price, standard reply, or pending-order RAG interrupt.",
    blockedStates: ["complaint", "handoff", "submittedOrder", "done", "optedOut", "anotherDatePurchase"],
    resolve: ({
      catalog,
      product,
  ...
  },
  {
    ...
    category: "knowledge",
    priority: ROUTING_PHASE.KNOWLEDGE,
    description: "Use legacy FAQ or RAG after stronger local knowledge and sales matches miss.",
    blockedStates: ["complaint", "handoff", "submittedOrder", "done", "optedOut", "anotherDatePurchase"],
    resolve: ({
      product,
      text,
...
function buildProductContextClarificationPlan({ customer = {}, productResolution = null, productSpecificQuestion = false } = {}) {
  if (!productSpecificQuestion || hasConfidentProductContext(productResolution)) return null;

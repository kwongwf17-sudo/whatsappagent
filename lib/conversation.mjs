...
import {
  ...
  getOpeningFlowDecision as getOpeningFlowDecisionFromHandler,
  hasOpeningFlowAlreadySent,
  rankProductCandidates,
  resolveProduct as resolveProductFromHandler,
} from "./opening_flow_handler.mjs";
import { buildOrderDraftPlan as buildOrderDraftPlanFromHandler } from "./order_handler.mjs";
import {
...
import {
  conversationActiveState as deriveConversationActiveState,
...
const PRODUCT_CONTEXT_CLARIFICATION_REPLY =
...
const UNKNOWN_PRODUCT_HANDOFF_REASON = "Do not know which product the customer wants to inquire about.";
const POLITE_CLOSE_REPLY =
  "Sama-sama \uD83D\uDE0A Boleh, nanti kalau kita kan order atau ada soalan lagi, WhatsApp saja kami di sini. Terima kasih!";
const DEFAULT_PENDING_ORDER_FORM_FOLLOWUPS = {
  sent: {},
  exhausted: false,
  exhaustedAt: "",
};
const EXPECTED_NEXT_ACTION_TTL_MS = 60 * 60 * 1000;
const EXPECTED_NEXT_ACTION_PACKAGE_INTEREST = "package_interest_confirmation";
...
export async function buildConversationPlan(args) {
  const source = args.source || {};
  const customer = args.customer || {};
  const productResolution = args.productResolution || resolveProduct(args.catalog, args.customerMessage, source, customer.productId);
  const openingFlowDecision =
    args.openingFlowDecision ||
    getOpeningFlowDecision({
      customer,
      productResolution,
      customerMessage: args.customerMessage,
      source,
      isFirstEligibleInbound: Boolean(args.isFirstEligibleInbound),
    });
  const basePlan = await buildBaseConversationPlan({ ...args, source, productResolution });
  return applyOpeningFlowDecision(basePlan, openingFlowDecision, { customer, source });
}

async function buildBaseConversationPlan({
  catalog,
  customer,
  customerMessage,
  source = {},
  ...
  const product = productResolution?.product || findProduct(catalog, customerMessage, source, customer.productId);
  const text = customerMessage.trim();
  if (customerHasSubmittedOrder(customer)) {
    customer = { ...customer, pendingOrder: null, awaitingPackageBInterest: false, expectedNextAction: null };
  }
  ...
  const decisionContext = {
    ...
    allowLocalSales,
    allowLocalKnowledge,
    orderDraft: earlyOrderDraft,
    pendingOrderAnswerInterrupt,
    buildErrorHandoffPlan: (_error, metadata) => buildUnhandledHandoffPlan({
      product,
      reason: `Routing rule failed: ${metadata.name || "unknown"}.`,
      severity: "high",
    }),
  };
  const decision = evaluateDecisionRules(baseDecisionRules, decisionContext);
  const plan = decision.plan || buildUnhandledHandoffPlan({ product });
  const normalizedPlan = normalizeConversationPlan(plan);
  const resolvedProductClarification =
    customer.awaitingProductClarification && hasConfidentProductContext(productResolution);
  const routedPlan = resolvedProductClarification
    ? {
        ...normalizedPlan,
        customerPatch: {
          ...(normalizedPlan.customerPatch || {}),
  ...
  return attachRoutingDecision(
    ...
    decision.plan
      ? decision
      : { ...decision, rule: "handoff", category: "handoff", description: "No eligible routing rule matched." },
    decisionContext
  );
}

const baseDecisionRules = [
  {
    name: "early_sales",
    category: "sales",
    priority: ROUTING_PHASE.SALES,
    description: "Handle sales prompt responses and clear sales objections before order or FAQ handling.",
    allowedStates: ["", "awaitingPackageInterest", "salesClosed"],
    blockedStates: ["complaint", "handoff", "pendingOrder", "submittedOrder", "done", "optedOut", "anotherDatePurchase"],
    resolve: ({
      catalog,
      product,
      text,
      ...
      faqSalesResponse,
      allowLocalSales,
      helpers: salesHandlerHelpers,
    }),
  },
  {
    name: "pending_order_reference",
    category: "order",
    priority: ROUTING_PHASE.ACTIVE_ORDER,
    description: "Continue a pending order when the customer refers to details sent earlier.",
    allowedStates: ["pendingOrder"],
    resolve: ({ product, text, customer, activeState, orderDraft }) => {
      if (activeState !== "pendingOrder" || !refersToPreviousOrderDetails(text)) return null;
      if (orderDraft.isComplete) {
        return buildOrderDraftPlan({
          customer,
          product,
          text,
          orderDraft,
  ...
  },
  {
    ...
    category: "order",
    priority: ROUTING_PHASE.ACTIVE_ORDER,
    description: "Collect or submit order details when the message has purchase intent.",
    blockedStates: ["complaint", "handoff", "submittedOrder", "done", "optedOut", "anotherDatePurchase", "pendingUpsell"],
    resolve: ({ product, text, customer, orderDraft, pendingOrderAnswerInterrupt, ragAnswer, routeClassification }) => {
  ...
  },
  {
    ...
    category: "closing",
    priority: ROUTING_PHASE.ACKNOWLEDGEMENT,
    description: "Treat neutral acknowledgement as no-op or pending-order reminder.",
    blockedStates: ["complaint", "handoff", "submittedOrder", "done", "optedOut"],
    resolve: ({ product, customer, text, activeState, orderDraft }) => buildAcknowledgementPlan({
      product,
      customer,
      text,
      activeState,
      orderDraft,
      helpers: closingHandlerHelpers,
    }),
  },
  {
    name: "polite_close",
    category: "closing",
    priority: ROUTING_PHASE.ACKNOWLEDGEMENT,
    description: "Reply politely to thanks or conversation-close messages.",
    blockedStates: ["complaint", "handoff", "submittedOrder", "done", "optedOut"],
    resolve: ({ product, text }) => buildPoliteClosePlan({
      product,
      text,
      closeReply: POLITE_CLOSE_REPLY,
      helpers: closingHandlerHelpers,
    }),
  },
  {
    name: "product_context_clarification",
    category: "clarification",
    priority: ROUTING_PHASE.CLARIFICATION,
    description: "Ask which product the customer means when a product question has no confident product context.",
    blockedStates: ["complaint", "handoff", "submittedOrder", "done", "optedOut", "anotherDatePurchase"],
    resolve: ({ customer, productResolution }) => buildProductContextClarificationPlan({
      customer,
      productResolution,
    }),
  },
  {
    ...
    description: "Answer general approved FAQ, standard reply, or pending-order RAG interrupt.",
    blockedStates: ["complaint", "handoff", "optedOut", "anotherDatePurchase"],
    resolve: ({
      catalog,
      product,
      text,
      ...
      allowLocalKnowledge,
      knowledgeSalesFollowup,
      helpers: knowledgeHandlerHelpers,
    }),
  },
  {
    name: "matched_sales",
    category: "sales",
    priority: ROUTING_PHASE.SALES,
    description: "Use approved sales reply matches from the classifier or vector layer.",
    allowedStates: ["", "awaitingPackageInterest", "salesClosed"],
    blockedStates: ["complaint", "handoff", "pendingOrder", "submittedOrder", "done", "optedOut", "anotherDatePurchase"],
    resolve: ({ product, customer, salesReplyMatch }) => buildMatchedSalesPlan({
      product,
      customer,
      salesReplyMatch,
      helpers: salesHandlerHelpers,
  ...
  },
  {
    ...
    category: "knowledge",
    priority: ROUTING_PHASE.KNOWLEDGE,
    description: "Use RAG after stronger local knowledge and sales matches miss.",
    blockedStates: ["complaint", "handoff", "optedOut", "anotherDatePurchase"],
    resolve: ({
      product,
      text,
  ...
  },
  {
    ...
    category: "clarification",
    priority: ROUTING_PHASE.CLARIFICATION,
    description: "Ask a short clarification question for ambiguous follow-ups.",
    blockedStates: ["complaint", "handoff", "submittedOrder", "done", "optedOut", "anotherDatePurchase"],
    resolve: ({ product, customer, text, conversationContext, routeClassification }) => buildClarificationPlan({
      product,
      customer,
      text,
      conversationContext,
  ...
  {
    name: "delivery_fallback",
    category: "order",
    priority: ROUTING_PHASE.FALLBACK,
    description: "Ask for full order details when delivery terms appear during pending order.",
    allowedStates: ["pendingOrder"],
    resolve: ({ product, customer, text }) => buildDeliveryFallbackPlan({
      product,
      customer,
      text,
      deliveryKeywords: DELIVERY_KEYWORDS,
      helpers: closingHandlerHelpers,
    }),
  },
];
validateDecisionRules(baseDecisionRules);
...
export function shouldSendProductOpeningFlow(customer = {}, product = null, customerMessage = "", source = {}, options = {}) {
  const text = String(customerMessage || "").trim();
  if (!product?.id || !text) return false;
  const productResolution = resolveProduct({ products: [product], default_product_id: product.id }, text, source, customer.productId);
  const decision = getOpeningFlowDecision({
    customer,
    productResolution,
    customerMessage: text,
    source,
    isFirstEligibleInbound: Boolean(options.isFirstEligibleInbound),
  });
  return decision.shouldSend;
}

export function resolveProduct(catalog, text = "", source = {}, fallbackProductId = "") {
  return resolveProductFromHandler({ catalog, text, source, fallbackProductId, findProductMatch });
}
...
export function getOpeningFlowDecision({
  ...
  return getOpeningFlowDecisionFromHandler({
    customer,
    productResolution,
    customerMessage,
    source,
    isFirstEligibleInbound,
    helpers: openingFlowHelpers,
  });
}
...
const openingFlowHelpers = {
  conversationActiveState,
  isProductNameMessage,
  isProductMentionedInText,
  productIntro,
  textMessage,
};
...
function buildProductContextClarificationPlan({ customer = {}, productResolution = null } = {}) {
  if (hasConfidentProductContext(productResolution)) return null;
  if (customer.awaitingProductClarification) {
    return buildUnhandledHandoffPlan({
      product: {},
      reason: UNKNOWN_PRODUCT_HANDOFF_REASON,
    });
  }
  return {
    customerPatch: {
    ...
    messages: [textMessage(PRODUCT_CONTEXT_CLARIFICATION_REPLY)],
    handoffRequired: false,
  };
}

function hasConfidentProductContext(productResolution = null) {
  if (!productResolution?.matched) return false;
...
function ragAnswerConversationPlan(ragAnswer, product, customer, earlyOrderDraft, pendingOrderAnswerInterrupt, knowledgeSalesFollowup = "") {
  ...
  return {
    ...
    messages:
      shouldUseFaqPrompt
        ? knowledgeAnswerMessages(ragAnswer.reply, product, customer, earlyOrderDraft, pendingOrderAnswerInterrupt, knowledgeSalesFollowup)
...
export function buildOrderDraftPlan({ customer = {}, product = {}, text = "", orderDraft = {} }) {
  return buildOrderDraftPlanFromHandler({
    customer,
    product,
    text,
    orderDraft,
    helpers: orderHandlerHelpers,
  });
}

const orderHandlerHelpers = {
...
const knowledgeHandlerHelpers = {
  findApprovedFaqLocalMatch,
  productPriceFaq,
  normalizeReplyText,
  routeAllowsStandardReply,
  findStandardReply,
  findFaqAnswer,
  knowledgeAnswerPatch,
  knowledgeAnswerMessages,
  ragAnswerConversationPlan,
  textMessage,
};

const salesHandlerHelpers = {
  findSalesReplyExactMatch,
  hasSalesObjectionLanguage,
  orderFormMessages,
  salesConversationClosedPatch,
  salesReplyFromMatch,
  salesReplyPlan,
...
const closingHandlerHelpers = {
  ambiguousQuestionClarification,
  buildUnhandledHandoffPlan,
  incompleteOrderMessages,
...
export function isProductSpecificQuestion(text) {
  ...
  return (
    ...
    /\b(cara\s*guna|cara\s*pengguna|macam\s*mana\s*guna|how\s*to\s*use|cara\s*pakai|how\s*to\s*apply|usage|instruction|pengguna(?:an)?|fungsi|function|what\s*(is|are).*(function|benefit)|benefit|kebaikan|ingredient|bahan|contains?|contain|spec|specification|berapa\s*ml|volume|size|suction|mode|head|usb|recharge|side\s*effect|allergy|sensitive|warranty|refund|rosak|price|harga|harge|brapa|brape|package|pakej|combo|stock|available|origin|made\s*in)\b/i.test(normalized)
  );
}
...
export function classifyFaqSalesPromptResponse(customer, customerMessage) {
  if (!isAwaitingPackageInterestConfirmation(customer)) return "";
  // Context-dependent confirmations are classified by the route classifier.
  // Keep this compatibility hook from turning short acknowledgements into orders
  // without considering the immediate previous bot/team message.
  return "";
}
...
export function findProductMatch(catalog, text = "", source = {}) {
  ...
  const candidates = rankProductCandidates(products, haystack, source && Object.keys(source).length ? "ad_metadata" : "message_text");
  if (candidates[0]?.confidence >= 0.75) return candidates[0].product;
  return null;
}
...
export function approvedFaqRecordsForProduct(catalog, product, options = {}) {
  ...
  const general = includeGeneral
    ...
    : [];
  const productFaqs = includeProduct
    ? (product?.approved_faqs || []).filter((item) => item.active !== false)
    : [];
  return [
...
export function findApprovedFaqPrimaryIntentMatch(catalog, product, routeClassification, options = {}) {
  const primaryIntent = String(routeClassification?.primaryIntent || "").trim();
  const messageType = String(routeClassification?.messageType || "").trim();
  const confidence = String(routeClassification?.confidence || "").trim();
  if (!primaryIntent || confidence === "low") return null;
  if (!["general_faq", "product_question"].includes(messageType)) return null;

  const records = approvedFaqRecordsForProduct(catalog, product, {
    includeGeneral: messageType === "general_faq",
    includeProduct: messageType === "product_question",
    faqLibrary: options.faqLibrary,
  });
  return (
    records.find((item) =>
      [item.id, item.topic_key, item.topicKey]
        .map((value) => String(value || "").trim())
        .some((value) => value && value === primaryIntent)
    ) || null
  );
}
...
export function findApprovedFaqExactMatch(catalog, product, text, options = {}) {
  ...
  const records = approvedFaqRecordsForProduct(catalog, product, {
    ...
    includeProduct: options.includeProduct,
    faqLibrary: options.faqLibrary,
  });
  return (
    records.find((item) =>
...
export function findApprovedFaqLocalMatch(catalog, product, text, options = {}) {
  ...
  return findApprovedFaqHeuristicMatch(records, product, text, { includeProduct: options.includeProduct });
}
...
export function findSalesReplyPrimaryIntentMatch(catalog, product, routeClassification, options = {}) {
  const primaryIntent = normalizeReplyText(routeClassification?.primaryIntent || "").replace(/\s+/g, "_");
  const messageType = String(routeClassification?.messageType || "").trim();
  const confidence = String(routeClassification?.confidence || "").trim();
  if (!primaryIntent || confidence === "low") return null;
  if (!["sales_reply", "purchase_intent"].includes(messageType)) return null;

  const records = salesReplyRecordsForProduct(catalog, product, {
    salesReplyLibrary: options.salesReplyLibrary,
  }).filter((item) => item.active !== false);
  const matches = records.filter((item) => salesReplyIntentKey(item) === primaryIntent);
...
function findApprovedFaqHeuristicMatch(records, product, text, options = {}) {
  const normalized = normalizeReplyText(text);
...
function productPriceFaq(product, normalizedText) {
  ...
  if (!/(harga|price|how\s*much|promo|promosi|offer|deal|b\\$|\$|\bberapa\b.*\b(unit|package|pakej|combo|satu|1)\b|\b(unit|package|pakej|combo|satu|1)\b.*\bberapa\b)/i.test(normalizedText)) return null;
...
function isNeutralAcknowledgement(text) {
  const normalized = normalizeReplyText(text)
    ...
    .trim();
  if (!normalized) return false;
  if (/\b(order|beli|ambil|proceed|lock|book|harga|price|berapa|package|pakej|cod|alamat|address|nama|name|phone|nombor|number)\b/i.test(normalized)) {
    return false;
  }
  const compact = normalized.replace(/\s+/g, " ");
  if (/^(terima kasih|thank you)(\s+(kita|sis|boss|bos|admin))?$/.test(compact)) return true;
...
function faqReplyMessages(answer, product, customer, knowledgeSalesFollowup = "") {
  const messages = [textMessage(answer)];
  if (shouldSendKnowledgeSalesPrompt(product, customer)) {
    if (knowledgeSalesFollowup !== false) {
      messages.push(textMessage(String(knowledgeSalesFollowup || "").trim() || salesPromptForProduct(product)));
    }
  }
  return messages;
}
...
function knowledgeAnswerMessages(answer, product, customer, orderDraft, pendingOrderAnswerInterrupt, knowledgeSalesFollowup = "") {
  const messages = pendingOrderAnswerInterrupt
    ? [textMessage(answer)]
    : faqReplyMessages(answer, product, customer, knowledgeSalesFollowup);
...
function faqSalesPromptPatch(product, customer) {
  if (conversationActiveState(customer) === "awaitingPackageInterest") {
    return {
      ...
      expectedNextAction: packageInterestExpectedNextAction(product),
    };
  }
  ...
  const awaitingPackageBInterest = shouldSendFaqSalesPrompt(product, customer);
  return {
    ...
    awaitingPackageBInterest,
    expectedNextAction: awaitingPackageBInterest ? packageInterestExpectedNextAction(product) : null,
  };
}

function packageInterestExpectedNextAction(product = {}) {
  return {
    type: EXPECTED_NEXT_ACTION_PACKAGE_INTEREST,
    source: "faq_sales_followup",
    productId: product?.id || "",
    setAt: new Date().toISOString(),
    expiresAt: new Date(Date.now() + EXPECTED_NEXT_ACTION_TTL_MS).toISOString(),
  };
}

function isAwaitingPackageInterestConfirmation(customer = {}) {
  if (customer.awaitingPackageBInterest) return true;
  const expected = customer.expectedNextAction;
  if (!expected || typeof expected !== "object") return false;
  if (expected.type !== EXPECTED_NEXT_ACTION_PACKAGE_INTEREST) return false;
  if (!expected.expiresAt) return true;
  const expiresAt = Date.parse(expected.expiresAt);
  return !Number.isFinite(expiresAt) || expiresAt > Date.now();
}

function suppressesKnowledgeSalesPrompt(customer) {
  return ["pendingOrder", "submittedOrder", "done", "handoff", "anotherDatePurchase", "salesClosed", "optedOut"].includes(
...
function salesReplyPlan(customer, product, salesReply) {
  ...
  const expectsPackageInterest = !closeAfterReply && salesReplyExpectsPackageInterest(salesReply);
  const packageInterestPatch = closeAfterReply
    ? { awaitingPackageBInterest: false, expectedNextAction: null }
    : expectsPackageInterest
      ? { awaitingPackageBInterest: true, expectedNextAction: packageInterestExpectedNextAction(product) }
      : {};
...
function incompleteOrderMessages(product, orderDraft) {
  const draft = orderDraftWithDetailFlags(orderDraft);
  if (draft.unsupportedQuantity || draft.unsupportedOrderOption) {
    return [textMessage(unsupportedOrderOptionReply(product, draft))];
  }
  if (!draft.hasAnyDetails) {
    return [textMessage(orderFormText(product))];
  }
  const missing = missingOrderFields(product, draft);
  if (!missing.length) {
    return [textMessage(orderFormText(product))];
  }
  return [textMessage(missingOrderReply(product, draft, missing))];
}

function orderDraftWithDetailFlags(orderDraft = {}) {
  return {
    ...orderDraft,
    hasAnyDetails: Boolean(
      orderDraft.hasAnyDetails ||
        orderDraft.orderOptionId ||
        orderDraft.packageId ||
        orderDraft.addOnChoice ||
        orderDraft.name ||
        orderDraft.phone ||
        orderDraft.address ||
...
function orderFormText(product) {
  ...
  return [
    ...
    orderFormFieldLine(form, "option"),
...
export function normalizeCustomerMessage(value) {
  return String(value || "")
    ...
    .replace(/\b(?:pakej|paket|pakage|packed|pack|pkg)\b/g, "package")
...
export function extractOrderDetails(text, product) {
  ...
  if (!orderOptionChoice) {
    orderOptionChoice = extractBareOrderOptionChoice(normalizedText, product);
  }
  ...
  const phone = fieldValue(normalizedText, uniqueFieldAliases([form.phoneLabel, "phone number", "phone", "tel", "telefon", "no telefon", "nombor telefon", "mobile", "whatsapp"])) || freeForm.phone;
...
function pendingOrderPatch(productId, existingPendingOrder, orderDraft) {
  return {
    ...
    formFollowups: normalizePendingOrderFormFollowups(existingPendingOrder?.formFollowups),
  };
}

function salesReplyExpectsPackageInterest(salesReply = {}) {
  const text = normalizeReplyText(salesReply.approved_reply || "");
  if (!text) return false;
  const asksDecision = /\b(minat|mau|mahu|nak|ambil|proceed|jadi|lock|hold|reserve|pilih|choose|which|mana|rasa)\b/i.test(text);
  const packageContext = /\b(package|pakej|paket|pkg|promo|combo|unit)\b/i.test(text);
  const questionContext = /[?]|\b(mana|which|pilih|choose|rasa|ada\s+kita)\b/i.test(text);
  return Boolean(asksDecision && packageContext && questionContext);
}

function normalizePendingOrderFormFollowups(value = {}) {
  const source = value && typeof value === "object" ? value : {};
  const sent = source.sent && typeof source.sent === "object" ? source.sent : {};
  return {
    ...DEFAULT_PENDING_ORDER_FORM_FOLLOWUPS,
    sent: { ...sent },
...
function isClearOrderOptionReply(text, orderDraft, product) {
  ...
  return terms.some((term) => orderOptionTermMatches(normalized, term, { allowContains: false }));
}
...
function shouldHandleOrderDraft(customer, text, orderDraft, product, ragAnswer, routeClassification = null) {
  ...
  if (isProductNameMessage(product, text)) return false;
...
export function conversationActiveState(customer = {}) {
  return deriveConversationActiveState(customer);
}
...
function hasSalesObjectionLanguage(text) {
  ...
  return /\b(mahal|expensive|too much|harga tinggi|over budget|overbudget|tak mampu|tidak mampu|cannot afford|cant afford|can't afford)\b/i.test(normalized) ||
    /\b(will\s+update|update|bagitau|bgtau|beritahu|inform|message|text|reply)\b.*\b(kalau|when|once|if)\b.*\bready\b.*\b(beli|buy|order|ambil)\b/i.test(normalized) ||
    /\b(kalau|when|once|if)\b.*\bready\b.*\b(beli|buy|order|ambil)\b/i.test(normalized) ||
    /\b(belum|tidak|not)\s+ready\b.*\b(beli|buy|order|ambil)\b/i.test(normalized) ||
...
function hasStrongOrderEvidence(text, draft = {}, product = null) {
  ...
  const hasAddressCue = /\b(spg|simpang|jalan|jln|kg|kpg|kampong|kampung|rpn|rumah|house|no\.?|unit|block|blok|lot|mukim|bandar|kb|tutong|temburong|brunei|muara|mentiri|mumong)\b/i.test(body);
...
function extractOrderOptionChoice(text, product) {
  ...
  return orderOptionsForProduct(product).find((item) => {
    ...
    const normalizedAliases = aliases.map((alias) => normalizeReplyText(alias)).filter(Boolean);
    const optionTokens = [id, shortPackage].filter(Boolean);
    const optionPattern = optionTokens.length
      ? new RegExp(`\\b(package|pakej|paket|pakage|packed|pack|pkg|option|opsyen|pilihan|order option)\\s*[:\\uFF1A-]?\\s*(?:${optionTokens.map(escapeRegExp).join("|")})\\b`, "i")
      : null;
    return (
      (labelledOption && normalizedAliases.some((alias) => orderOptionTermMatches(normalizeReplyText(labelledOption), alias, { allowContains: false }))) ||
      (optionPattern && optionPattern.test(text)) ||
      normalized.includes(`order package ${id}`) ||
      normalized.includes(`package ${id}`) ||
      normalizedReply.includes(`package ${id}`) ||
      ...
      normalizedAliases.some((alias) => alias && alias.length > 1 && orderOptionTermMatches(normalizedReply, alias))
    );
  });
}

function extractBareOrderOptionChoice(text, product) {
  const choices = normalizeOrderFormText(text)
    .split(/\r?\n|[,;|]/)
    .map((line) => normalizeReplyText(cleanupOrderDetailValue(line)))
    .filter(Boolean);
  return orderOptionsForProduct(product).find((item) => {
    const name = normalizeReplyText(item.name);
    const id = normalizeReplyText(item.id);
    const shortPackage = name.match(/^package\s+([a-z0-9]+)$/i)?.[1] || "";
    const aliases = [name, id, shortPackage, ...(item.aliases || []).map((alias) => normalizeReplyText(alias))].filter(Boolean);
    return choices.some((choice) =>
      aliases.some((alias) => alias.length > 0 && orderOptionTermMatches(choice, alias, { allowContains: false }))
    );
  }) || null;
}

function orderOptionTermMatches(input, term, options = {}) {
  const normalizedInput = normalizeReplyText(input);
  const normalizedTerm = normalizeReplyText(term);
  if (!normalizedInput || !normalizedTerm) return false;
  if (normalizedInput === normalizedTerm) return true;
  if (options.allowContains !== false && normalizedTerm.length > 1 && normalizedInput.includes(normalizedTerm)) return true;
  return isFuzzyPackageOptionMatch(normalizedInput, normalizedTerm);
}

function isFuzzyPackageOptionMatch(input, term) {
  const inputMatch = input.match(/^([a-z]+)\s+([a-z0-9])$/i);
  const termMatch = term.match(/^([a-z]+)\s+([a-z0-9])$/i);
  if (!inputMatch || !termMatch) return false;
  if (inputMatch[2] !== termMatch[2]) return false;
  if (termMatch[1] !== "package") return false;
  return levenshteinDistance(inputMatch[1], termMatch[1]) <= 2;
}
...
function extractFreeFormOrderDetails(text) {
  ...
  const parsed = {
    name: nameLineIndex >= 0 ? lines[nameLineIndex] : inferNameFromPhoneLine(lines[phoneLineIndex]),
    phone,
    address: cleanupOrderDetailValue(addressLines.join(" ")),
  };
...
function parseMixedLineOrderDetails(line) {
  ...
  const withoutPhone = phoneMatch
    ...
    : original;
  const cleaned = cleanupOrderDetailValue(withoutPhone.replace(/\s+/g, " ").trim());
  const labelled = parseInlineLabelledOrderDetails(cleaned);
  if (labelled.name || labelled.address) {
    return { ...labelled, phone };
  }

  const dashed = parseSegmentedOrderDetails(cleaned);
  if (dashed.name || dashed.address) {
    return { ...dashed, phone };
  }

  const addressMatch = cleaned.match(addressCuePattern());
  if (!addressMatch) {
    return { phone };
  }
  const matchedAddressCue = String(addressMatch[1] || "").toLowerCase();
  const hasStrongAddressCue = strongAddressCuePattern().test(cleaned);
  if (matchedAddressCue === "unit" && extractRequestedUnitQuantity(original) && !hasStrongAddressCue) {
    return { phone };
  }

  const beforeAddress = cleanupOrderDetailValue(cleaned.slice(0, addressMatch.index));
  let address = cleanupOrderDetailValue(cleaned.slice(addressMatch.index));
  ...
  if (!name) {
    ...
    if (trailingName && isLikelyNameLine(trailingName)) {
      name = trailingName;
      address = cleanupOrderDetailValue(address.slice(0, address.length - trailingName.length));
    }
  }
...
function parseInlineLabelledOrderDetails(text) {
  const clean = String(text || "").trim();
  if (!clean) return {};
  const name = cleanupOrderDetailValue(
    clean.match(/(?:^|[,;-]\s*)nama\s+(.+?)(?=\s+(?:alamat|address|phone|telefon|tel|whatsapp|no\s+\d|nombor)\b|[,;-]|$)/i)?.[1] || ""
  );
  const address = cleanupOrderDetailValue(
    clean.match(/(?:^|[,;-]\s*)(?:alamat|address)\s+(.+?)(?=\s+(?:phone|telefon|tel|whatsapp|no\s+\d|nombor|pilihan|package|pakej|paket|pkg)\b|[,;-]|$)/i)?.[1] || ""
  );
  return {
    name: isLikelyNameLine(name) ? name : "",
    address: address && isLikelyAddressLine(address) ? address : "",
  };
}

function parseSegmentedOrderDetails(text) {
  const parts = String(text || "")
    .split(/\s+-\s+|[|;]/)
    .map((part) => cleanupOrderDetailValue(part))
    .filter(Boolean);
  if (parts.length < 2) return {};
  const address = parts.find((part) => isLikelyAddressLine(part)) || "";
  const name = parts.find((part) =>
    part !== address &&
    isLikelyNameLine(part) &&
    !/^(package|pakej|paket|pakage|packed|pack|pkg|option|opsyen|pilihan)\b/i.test(part)
  ) || "";
  return { name, address };
}

function cleanupOrderDetailValue(value) {
  return String(value || "")
    .replace(/^[\s,;:-]+/g, "")
    .replace(/[\s,;:-]+$/g, "")
    .replace(/\s+/g, " ")
...
function inferNameFromPhoneLine(line) {
  const original = String(line || "").trim();
  if (!original) return "";
  const phoneMatch = findPhoneMatch(original);
  if (!phoneMatch) return "";
  const withoutPhone = cleanupOrderDetailValue(
    `${original.slice(0, phoneMatch.index)} ${original.slice(phoneMatch.index + phoneMatch.raw.length)}`
  );
  return isLikelyNameLine(withoutPhone) ? withoutPhone : "";
}
...
function isLikelyAddressLine(line) {
  const text = String(line || "").trim();
  if (!text) return false;
  const hasAddressKeyword = addressCuePattern().test(text);
  if (hasAddressKeyword) return true;
  return false;
}

function addressCuePattern() {
  return /\b(no\.?|simpang|spg|jalan|jln|kg|kpg|kampong|kampung|rpn|stkrj|rumah|house|unit|block|blk|lot|mumong|kiulap|tutong|kb|kuala\s*belait|temburong|brunei|bandar|seria|jerudong|rimba|lambak|berakas|mengkubau|mulaut|sengkurong|lumut|liang)\b/i;
}

function strongAddressCuePattern() {
  return /\b(no\.?|simpang|spg|jalan|jln|kg|kpg|kampong|kampung|rpn|stkrj|rumah|house|block|blk|lot|mumong|kiulap|tutong|kb|kuala\s*belait|temburong|brunei|bandar|seria|jerudong|rimba|lambak|berakas|mengkubau|mulaut|sengkurong|lumut|liang)\b/i;
}

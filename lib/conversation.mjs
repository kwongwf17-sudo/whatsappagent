...
export async function buildConversationPlan({
  ...
  if (shouldHandleOrderDraft(customer, text, earlyOrderDraft, product, ragAnswer, routeClassification)) {
  ...
  const salesReply = allowLocalSales
  ...
  if (shouldHandleOrderDraft(customer, text, orderDraft, product, ragAnswer, routeClassification)) {
...
function shouldHandleOrderDraft(customer, text, orderDraft, product, ragAnswer, routeClassification = null) {
  const hasBuyingIntent = ragAnswer?.buyingIntent === "buying" || routeIndicatesPurchaseIntent(routeClassification);
  if (customer?.pendingOrder) {
    return Boolean(orderDraft.isComplete || orderDraft.hasNewDetails || hasOrderFormFields(orderDraft) || isOrderStartIntent(text) || hasBuyingIntent);
  }
  if (isOrderStartIntent(text) || hasBuyingIntent) return true;

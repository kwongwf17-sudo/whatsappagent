...
test("team content can use account and per-product table storage", async () => {
  ...
  try {
    ...
    const deleted = await store.deleteContent("ALLEN");
    assert.equal(deleted.deleted, 3);
    assert.equal(adapter.accounts.has("ALLEN"), false);
    assert.equal(adapter.products.has("ALLEN"), false);
    assert.equal(adapter.accounts.has("allen"), true);
  } finally {
    await rm(dataDir, { recursive: true, force: true });
  }
...
class MemoryTeamContentTableAdapter {
  ...
  async deleteTeamContentRecord(accountId) {
    const accountDeleted = this.accounts.delete(accountId) ? 1 : 0;
    const productsDeleted = this.products.has(accountId) ? this.products.get(accountId).length : 0;
    this.products.delete(accountId);
    return accountDeleted + productsDeleted;
  }

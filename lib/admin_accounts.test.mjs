...
test("business admin passwords are hashed and access can be managed by super admin", async () => {
  ...
  try {
    ...
    assert.equal(controlled.autoReplyMode, "paused");
    assert.equal(controlled.testMode, true);
    assert.equal((await store.getAccount("demo")).automationPaused, true);
    const flowsOnly = await store.setOperationalControl("demo", {
      autoReplyMode: "flows_only",
      automationPaused: false,
      testMode: false,
    });
    assert.equal(flowsOnly.autoReplyMode, "flows_only");
    assert.equal(flowsOnly.automationPaused, false);
    ...
    const updatedSettings = await store.updateTeamSettings("demo", {
      ...
      openaiApiKey: "sk-team-openai-key",
      openaiVectorStoreId: "vs_team",
      openaiModel: "gpt-5",
      followupSendsPerMinute: 25,
      followupIntervalMinutes: 2,
      openingFlowWindowEnabled: true,
      openingFlowWindowStart: "08:30",
      openingFlowWindowEnd: "22:15",
    ...
    assert.equal(updatedSettings.settings.assetsBaseUrl, "https://cdn.example.com");
    ...
    assert.equal(updatedSettings.settings.openaiApiKey, "sk-...key");
    assert.equal(updatedSettings.settings.openaiModel, "gpt-5");
    assert.equal((await store.getTeamSettings("demo")).whatsappAccessToken, "secret-team-token");
    assert.equal((await store.getTeamSettings("demo")).openaiApiKey, "sk-team-openai-key");
    assert.equal((await store.getTeamSettings("demo")).openaiModel, "gpt-5");
    assert.equal((await store.getTeamSettings("demo")).openingFlowWindowEnabled, true);
    assert.equal((await store.getTeamSettings("demo")).openingFlowWindowStart, "08:30");
    assert.equal((await store.getTeamSettings("demo")).openingFlowWindowEnd, "22:15");
    assert.equal(updatedSettings.settings.handoffAlertEnabled, true);
    assert.equal(updatedSettings.settings.handoffAlertNumber, "6731234567");
    assert.equal(updatedSettings.settings.handoffAlertCooldownMinutes, 7);
    assert.equal((await store.getTeamSettings("demo")).handoffAlertNumber, "6731234567");
    await assert.rejects(
      store.updateTeamSettings("demo", { openaiApiKey: "vs_wrong_field" }),
      /OpenAI API Key.*vector store ID/
    );
    await assert.rejects(
      store.updateTeamSettings("demo", { openaiVectorStoreId: "sk-wrong-field" }),
      /Vector Store ID.*OpenAI API key/
    );
    const rawWithSettings = await readFile(path.join(dataDir, "admin_accounts.json"), "utf8");
    assert.doesNotMatch(rawWithSettings, /secret-team-token|sk-team-openai-key/);
    ...
    const deleted = await store.deleteAccount("second-store");
    assert.equal(deleted.id, "second-store");
    assert.equal(await store.getAccount("second-store"), null);
    await assert.rejects(store.deleteAccount("missing-store"), /Account not found/);
  } finally {

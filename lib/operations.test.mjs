...
test("purgeAccountData removes scoped operational json records", async () => {
  const dataDir = await mkdtemp(path.resolve("whatsapp_agent/data/.operations-test-"));
  try {
    const store = new OperationsStore(dataDir);
    await store.recordError({ scope: "test", message: "a", accountId: "store-a" });
    await store.recordError({ scope: "test", message: "b", accountId: "store-b" });
    await store.recordFailedMessage({ businessAccountId: "store-a", to: "1", messages: ["a"] });
    await store.recordFailedMessage({ businessAccountId: "store-b", to: "2", messages: ["b"] });
    await store.resolveNoReply({ businessAccountId: "store-a", customerId: "1", inboundMessageId: "in-a" });
    await store.resolveNoReply({ businessAccountId: "store-b", customerId: "2", inboundMessageId: "in-b" });
    await store.enqueueFollowups([
      { businessAccountId: "store-a", customerId: "1", followupKey: "day_1", message: "a" },
      { businessAccountId: "store-b", customerId: "2", followupKey: "day_1", message: "b" },
    ]);
    await store.updateDashboardProfile({ accountId: "store-a", name: "Store A", accentColor: "#111111" });

    const summary = await store.purgeAccountData("store-a");
    assert.equal(summary.errors, 1);
    assert.equal(summary.failedMessages, 1);
    assert.equal(summary.noReplyReviews, 1);
    assert.equal(summary.followupQueue, 1);
    assert.equal(summary.dashboardProfile, 1);
    assert.equal((await store.listErrors("store-a")).length, 0);
    assert.equal((await store.listErrors("store-b")).length, 1);
    assert.equal((await store.listFollowupQueue("store-a")).length, 0);
    assert.equal((await store.listFollowupQueue("store-b")).length, 1);
  } finally {
    await rm(dataDir, { recursive: true, force: true });
  }
});

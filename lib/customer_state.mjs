export function deriveCustomerState(customer = {}) {
  ...
  const hasSubmittedOrder = customerHasSubmittedOrder(customer);
  const hasOrderSubmissionHandoffMarker =
    !customer.humanRequired &&
    (!handoffReason || handoffReason === "customer submitted complete order details.");
  ...
  if (hasSubmittedOrder && hasOrderSubmissionHandoffMarker) {
  ...
  if (customer.pendingUpsell) {
    return { ...state, orderState: "pending_upsell", activeState: "pendingUpsell" };
  }

...
const config = {
  ...
  ordersTableEnabled: parseBool(getEnv("WHATSAPP_ORDERS_TABLE_ENABLED", "false")),
  ordersTableName: getEnv("WHATSAPP_ORDERS_TABLE", "orders"),
  processedMessagesTableEnabled: parseBool(getEnv("WHATSAPP_PROCESSED_MESSAGES_TABLE_ENABLED", "false")),
  processedMessagesTableName: getEnv("WHATSAPP_PROCESSED_MESSAGES_TABLE", "processed_messages"),
  pendingBuffersTableEnabled: parseBool(getEnv("WHATSAPP_PENDING_BUFFERS_TABLE_ENABLED", "false")),
  pendingBuffersTableName: getEnv("WHATSAPP_PENDING_BUFFERS_TABLE", "pending_buffers"),
  operationalEventsTableEnabled: parseBool(getEnv("WHATSAPP_OPERATIONAL_EVENTS_TABLE_ENABLED", "false")),
  operationalEventsTableName: getEnv("WHATSAPP_OPERATIONAL_EVENTS_TABLE", "operational_events"),
  auditEventsTableEnabled: parseBool(getEnv("WHATSAPP_AUDIT_EVENTS_TABLE_ENABLED", "false")),
  auditEventsTableName: getEnv("WHATSAPP_AUDIT_EVENTS_TABLE", "audit_events"),
  teamContentTableEnabled: parseBool(getEnv("WHATSAPP_TEAM_CONTENT_TABLE_ENABLED", "false")),
...
const postgresAdapter = config.storeBackend === "postgres"
  ? new PostgresJsonAdapter(config.dataDir, {
      ...
      ordersTableEnabled: config.ordersTableEnabled,
      ordersTableName: config.ordersTableName,
      processedMessagesTableEnabled: config.processedMessagesTableEnabled,
      processedMessagesTableName: config.processedMessagesTableName,
      pendingBuffersTableEnabled: config.pendingBuffersTableEnabled,
      pendingBuffersTableName: config.pendingBuffersTableName,
      operationalEventsTableEnabled: config.operationalEventsTableEnabled,
      operationalEventsTableName: config.operationalEventsTableName,
      auditEventsTableEnabled: config.auditEventsTableEnabled,
      auditEventsTableName: config.auditEventsTableName,
      teamContentTableEnabled: config.teamContentTableEnabled,
      teamContentAccountsTableName: config.teamContentAccountsTableName,
      teamContentProductsTableName: config.teamContentProductsTableName,
    })
  : null;
...
async function processInboundMessageCore({
  ...
  const outbound = openingFlowAlreadyReserved
    ? []
    : clampMessages(order ? orderSubmittedCustomerMessages(product) : (flowsOnlyMode ? sendPlan.messages : (repeatMessages ?? sendPlan.messages)));
